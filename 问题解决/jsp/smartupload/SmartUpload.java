...
public class SmartUpload
{
        ...
        throws SmartUploadException, IOException, ServletException
    {
        ...
        do
        {
            ...
            {
                String value = new String(m_binArray, m_startData, (m_endData - m_startData) + 1,getChartSet());
        ...
        throws SmartUploadException, IOException, ServletException
    {
         ...
         * add by stephen 
         */
        String outFileName = null;//输出的文件名
        
        if(destFileName == null){
        	outFileName = encodeFileName(getFileName(sourceFilePathName));
        	m_response.setHeader("Content-Disposition", "attachment; filename=".concat(outFileName));
        }else if(destFileName.length() == 0)
            m_response.setHeader("Content-Disposition", "attachment;");
        else{
        	outFileName = encodeFileName(destFileName);
        	m_response.setHeader("Content-Disposition", "attachment; filename=".concat(outFileName));
        }
    ...
    private String getDataHeader()
    {
        ...
        try {
            dataHeader = new String(m_binArray, start, (end - start) + 1,getChartSet());
