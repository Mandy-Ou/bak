...
var ComboxControl = {
	 ...
	 * @return {}
	 */
	getGuaGrid : function(cfg){
		if(!cfg.fieldLabel){
			cfg.fieldLabel = '担保人';				
		}
		if(!cfg.name){
			cfg.name = "guarantor";
			ExtUtil.error({msg:'参数必须传入 name 属性值,否则系统将以 guarantor 代替!'});
		}
		if(cfg.hasOwnProperty("allowBlank") && !cfg.allowBlank) cfg.fieldLabel = FormUtil.REQUIREDHTML + cfg.fieldLabel;
		/*担保人*/
		var barItems = [{type:'label',text:'担保人'},{type:'txt',name:'guarantor'}];
		var structure = [{header: '担保人',name: 'guarantor',width: 125},
			{header: '担保人意见',name: 'opinion',width: 200},
			{header: '法定/授权代表人',name: 'legal',width: 155},
			{header: '签字日期',name: 'signDate',width: 125}
			];					
		var gridCfg = {
		    fieldLabel: cfg.fieldLabel,
		    name: cfg.name,
			...
			switch(parseInt(isRequired)){
				case 0 : {
					...
					if(oldLabel && oldLabel.indexOf('*') != -1){
						break;
					}else  if(oldLabel && oldLabel.indexOf('*') == -1){
...
//		}
	...
	getDetailHtmlArr : function(params,htmArr){
		...
		if(htmlsCfg){
			...
			htmlsCfg =  {htms:this.replaceCustFieldsHtmls(htmArr,"")};
		}
		return htmlsCfg;
	},
	/**
