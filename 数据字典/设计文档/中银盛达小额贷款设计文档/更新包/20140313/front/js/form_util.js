...
//			border : false,buttonAlign : 'center'},cfg);
	 ...
	 * 获取百分比，千分比下拉框
	 */
	getUnitLCbox : function(fieldLabel,name){
		var rate_default_unit = SYSPARAMS_DATA["RATE_DEFAULT_UNIT"];
		if(!rate_default_unit) rate_default_unit = 1;/*如果系统参数中没有设置默认利率单位，就默认是 % 显示*/	
		var cbo_unint = FormUtil.getLCboField({fieldLabel:fieldLabel,name:name,
			data:Lcbo_dataSource.rateUnit_datas,value:rate_default_unit,
			allowBlank : false,width:45
		});
		return cbo_unint;
	},
	/**
	 * 获取利率类型[固定值,上浮]
	 */
	getRateTypeLCbox : function(fieldLabel,name){
		var cbo_rateType = FormUtil.getLCboField({fieldLabel:fieldLabel,name:name,
			data:Lcbo_dataSource.rateType_datas,value:1,/*默认固定值*/
