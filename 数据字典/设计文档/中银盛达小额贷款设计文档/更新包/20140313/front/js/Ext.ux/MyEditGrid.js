...
//		this.autoExpandColumn="remark";
	...
	*/
	...
	/**
	 * 添加多笔记录
	 * @param datas 要加入到Grid中的 json 对象数组 例：var dataArr = [{name:"张三",age:28},{name:"李四",age:35}]
	 * @param isappend 是否追加 [true:追加到Grid中,false:删除Grid中的数据，再将新数据插入到Grid中]
	 */
	addRecords : function(datas,isappend){
		if(!datas || datas.length == 0) return;
		var store = this.getStore();
		if(!isappend && store.getCount()>0) store.removeAll();
