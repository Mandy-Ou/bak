...
define(function(require, exports) {
	exports.viewUI = {
		...
		setRatesToForm : function(){
			...
			data.isadvance = 0;
			this.applyPanel.setJsonVals(data);
			var default_unint = SYSPARAMS_DATA["RATE_DEFAULT_UNIT"];
			if(!default_unint) default_unint = 1;/*如果系统参数中没有设置默认利率单位，就默认是 % 显示*/	
			var default_ftype = 1;/*固定值*/
			
			this.applyPanel.setFieldValue('compt_rate',{rate:data.rate,unint:default_unint});
			this.applyPanel.setFieldValue('compt_mrate',{mrate:data.mrate,munint:default_unint});
			this.applyPanel.setFieldValue('compt_urate',{utype:default_ftype,urate:data.urate,uunint:default_unint});
			this.applyPanel.setFieldValue('compt_frate',{ftype:default_ftype,frate:data.frate,funint:default_unint});
			this.applyPanel.setFieldValue('compt_prate',{prate:data.prate,punint:default_unint});
			this.applyPanel.setFieldValue('compt_arate',{arate:data.arate,aunint:default_unint});
...
//				});
			...
			var cbo_sourceType = FormUtil.getRCboField({
					fieldLabel : '业务来源方式',
					name : 'sourceType',
					"width" : 125,
					register : REGISTER.GvlistDatas,
			...
			var txt_realMan = FormUtil.getTxtField({
				fieldLabel : '实际用款人',
				name : 'realMan',
				"width" : "125"
			});
			...
			var fset_1 = FormUtil.createLayoutFieldSet({
						...
						fields : [comp_code, rad_loanMain, cbo_inType,
								rad_loanType,comb_mainLoanType,rad_limitType, comp_loanLimit,txt_appAmount,
								 txt_breed,compt_payType,txt_borBank,txt_borAccount]
					}, {
						cmns : FormUtil.CMN_THREE,
						fields : [ txt_realMan,txt_referrals,guaComboxAppGird,bdat_appdate,txt_managerName, cbo_sourceType]
					}]);
			...
			var compt_rate = FormUtil.getMyCompositeField({
				fieldLabel : '贷款利率',
				sigins : null,
				"allowBlank" : false,
			...
			var fset_2 = FormUtil.createLayoutFieldSet({
				...
				cmns : FormUtil.CMN_TWO,
				fields : [rad_rateType, compt_rate, rad_mgrtype,
						compt_mrate,  compt_urate, compt_frate,compt_prate,compt_arate]
			}]);
		...
		validApplyForm : function() {
			...
			// 贷款利率
			var rate_val = this.applyPanel.getValueByName("compt_rate");
			if (!rate_val || parseFloat(rate_val.rate) <= 0) {
			...
			// 管理费率
			var mrate_val = this.applyPanel.getValueByName("compt_mrate");
			if (mgrtype_val < 0) {
				errMsg[errMsg.length] = "管理费收取方式不能为空!";
			} else {
				if (mgrtype_val == 1 && (!mrate_val || mrate_val.mrate <= 0)) {
