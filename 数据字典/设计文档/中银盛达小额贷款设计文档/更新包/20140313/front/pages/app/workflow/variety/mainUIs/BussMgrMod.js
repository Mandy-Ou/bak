...
//			tab.setHeight(this.wh[1]);
			...
			if(this.finishBussCallback) tab.finishBussCallback = this.finishBussCallback;
			if(this.unFinishBussCallback) tab.unFinishBussCallback = this.unFinishBussCallback;
			...
			if(loadType && loadType == 1){	//标准 localXHR 方式加载
				...
				this.loadUiBysea(jsArr,tab,tabId,menuData);
			}
		},
		...
		loadUiBysea : function(url,tab,tabId,node){
			...
			Cmw.importPackage(url,function(module) {
					...
					tab = _this.getModelParamsToTab(tab,tabId,node);
				 	...
				 	var tabHeight = tab.getHeight();
				 	var tabWidth = tab.getWidth();
				 	Cmw.print("tabWidth="+tabWidth+", tabHeight="+tabHeight);
				 	tab.add(viewUI);
				 	tab.doLayout();
					//TabPannel 改变时，重新初始化
					_this.fireModeListeners(tab,model);
