...
define(function(require, exports) {
	exports.moduleUI = {
		...
		htmlArrs : function(that){
			...
			if(!_this.divObj){
				htmlArrs.push(['<div style="magin-bottom:10%;overFlow: auto;height : "'+height+'"; id="'+_this.indexdiv+'">'].join(" "));
			}
			 htmlArrs.push([
					...
					for(var i = 0;i<json_data.length;i++){
						var itemas = json_data[i];
								...
								if(size){
									...
									var formId2 = itemas.formId2;
									var checkBoxName = "CheckboxGroup_"+formId2;
									var boxs = Cmw.getElesByName(checkBoxName);
									if(result.indexOf(",")==-1){
										if(boxs[0] && result=="0"){
											boxs[0].checked  = true;
									...
									if(remarkvlue && remarkvlue !=""){
										var remark = "remark_"+formId2;
										var getVlueRemark = Ext.get(remark);
										if(getVlueRemark){
											getVlueRemark.dom.value = remarkvlue;
										}
									}
									matResultIds.push(formId2);
								}
							}
