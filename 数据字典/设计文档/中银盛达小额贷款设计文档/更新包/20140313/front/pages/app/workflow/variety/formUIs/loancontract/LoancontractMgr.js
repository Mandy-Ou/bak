		...
		var htmlArrs_1 = [
				'<tr>' +
							...
							'<tr id="'+_this.idMgr.flexDatasTrId+'" style="display:none;">' +
								'<th col="flexDatas">灵活还本信息</th> <td id="'+_this.idMgr.flexDatasTdId+'" colspan=5>&nbsp;</td>' +
							'</tr>', 
							'<tr height = "80">' +
...
//		    nextUrl: '#NEXTURLCFG#',
	    ...
	    callback: {
	        sfn: function(jsonData) {
	        	...
	        	_this.globalMgr.showFlexPlanInfos(_this,payType,flexDatas);
	...
	globalMgr : {
		 ...
		 * 显示灵活还本信息
		 */
		showFlexPlanInfos : function(_this,payType,flexDatas){
			var flag = false;
			if(payType == "P0008"){/*灵活还本*/
				flag = true;
			}
			var flexTr = Ext.get(_this.idMgr.flexDatasTrId);
			if(!flexTr) return;
			if(flag){
				flexTr.show();
				if(!flexDatas) return;
				...
				var flexTd = Ext.get(_this.idMgr.flexDatasTdId);
				if(!flexTd) return;
