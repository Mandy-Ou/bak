...
define(function(require, exports) {
	exports.viewUI = {
		...
		createCmpts : function(){
			...
			this.appMainPanel = new Ext.Panel({autoScroll:true,height:height,items:[this.customerPanel,this.applyPanel]});
		},
		createCustomerPanel : function() {
			var htmlArrs_1 = [
					...
					'<tr><th col="borBank">借款人银行</th> <td col="borBank" >&nbsp;</td><th col="borAccount">借款人账号</th> <td col="borAccount" >&nbsp;</td><th col="accName">帐户户名</th> <td col="accName" >&nbsp;</td></tr>'];
			var detailCfgs_1 = [{
			    cmns: 'THREE',
			    /* ONE , TWO , THREE */
			    model: 'single',
		...
		createFormPanel : function(){
			var _this = this;
			var txt_id = FormUtil.getHidField({fieldLabel: '豁免申请单ID', name: 'id'});
			...
			var txt_code = FormUtil.getHidField({fieldLabel: '编号',name: 'code'});
			
			var txt_totalAmount = FormUtil.getHidField({fieldLabel: '豁免息费合计', name: 'totalAmount'});
			
			var txt_managerId = FormUtil.getHidField({fieldLabel: '经办人ID',name: 'managerId'});
			...
			var cbo_etype = FormUtil.getLCboField({
			    ...
			    listeners : {change:function(field,newVal,oldVal){_this.showOrHidControls(newVal);}}
			});
			...
			var chk_exeItems = FormUtil.getCheckGroup({
			    ...
			    listeners : {change:function(){_this.showOrHidHeaders();}}
			});
			...
			var cmpt_startEndDate = FormUtil.getMyCompositeField({
				...
				hidden : true,
				width : 220,
				items : [bdat_startDate, {xtype : 'displayfield',value : '至'}, bdat_endDate]
				});
			...
			var layout_fields = [
			txt_id, txt_contractId, txt_procId, txt_breed, txt_code, txt_totalAmount, txt_managerId, {
		...
		createAttachMentFs : function(){
			 ...
			 *  ---- */
			this.attachMentFs = new  Ext.ux.AppAttachmentFs({title:'附件上传',isLoad:false,dir : 'extension_path',isSave:true});
		},
		/**
		 * 创建手续费豁免Grid
		 */
		createFreeGrid : function(){
			...
			var txt_cfat = FormUtil.getMoneyField({ fieldLabel: '豁免手续费',name: 'cfat'});
		...
		createPrepaymentGrid : function(){
			...
			var txt_fat = FormUtil.getMoneyField({ fieldLabel: '豁免提前还款手续费',name: 'fat'});
			var txt_rat = FormUtil.getMoneyField({ fieldLabel: '豁免利息',name: 'rat'});
			var txt_mat = FormUtil.getMoneyField({ fieldLabel: '豁免管理费',name: 'mat'});
			var txt_pat = FormUtil.getMoneyField({ fieldLabel: '豁免罚息',name: 'pat'});
			var txt_dat = FormUtil.getMoneyField({ fieldLabel: '豁免滞纳金',name: 'dat'});
			...
			this.prepaymentGrid = new Ext.ux.grid.MyEditGrid({
			    ...
			    isLoad: false,
			     height:300,
			    keyField: 'id',
			    editEles : {2:txt_fat,4:txt_rat,6:txt_mat,8:txt_pat,10:txt_dat},
			    listeners : {
			    	afteredit : function(e){ _this.showOrHidHeaders();}
			    }
			});
		},
		/**
		...
		createPlanGrid : function(){
			...
			this.planGrid = new Ext.ux.grid.MyEditGrid({
			    ...
			    height:300,
			    keyField: 'id',
			    editEles : {3:txt_rat,5:txt_mat,7:txt_pat,9:txt_dat},
			    listeners : {
			    	afteredit : function(e){ _this.showOrHidHeaders();}
			    }
			});
		...
		getSumTitleCfg : function(){
			...
			var labelHtml = '&nbsp;&nbsp;<span id="'+sumLabelId+'" style="color:red;font-weight:bold;">汇总信息</span>';
			return {id:sumLabelId,html : labelHtml}
		},
		/**
		 * 打开展期客户选择窗口
		 */
		...
		showCustomerDialog : function(){
			...
			if(this.customerDialog){
				...
				var _this = this;
				Cmw.importPackage('pages/app/dialogbox/ExemptDialogbox',function(module) {
		...
		getBatchDatas : function(){
			...
			switch(parseInt(etype)){
				case 1 :{/*放款手续费豁免*/
					...
					batchDatas[batchDatas.length] = {formId:formId,fat:fat,totalAmount:totalAmount};
					break;
				}case 2 :{/*提前还款息费豁免*/
					var record = store.getAt(0);
		...
		getExtItemFields : function(exeItems){
			if(Ext.isNumber(exeItems)) exeItems = exeItems.toString();
				...
				sfn : function(formDatas) {
					...
					if (submitType == 0) {
						...
						var procId = _this.applyPanel.getValueByName("procId");
						 ExtUtil.confirm({title:'提示',msg:'确定提交编号为："'+code+'"的展期申请单?',fn:function(){
		...
		checkPlanGridData : function(){
			...
			if(count < 0){
				...
				for(var i=0; i<count; i++){
					...
					var tat = record.get("tat") || 0;
					if(parseFloat(tat) > parseFloat(zytotalAmount)){
						errMsg[errMsg.length] = "正常/逾期息费豁免列表中的第<b>"+phases+"</b>期的豁免合计金额为不能超过应收合计金额!";
					}
				}
			}
			return errMsg;
		...
		loadDatas : function(){
			...
			if(optionType && optionType == OPTION_TYPE.EDIT){/*修改*/
				...
				this.applyPanel.setValues('./fcExempt_get.action', {params : {id : applyId},sfn: function(json_data){
				...
				this.applyPanel.setValues('./fcExempt_add.action', {sfn: function(json_data){
		...
		getAttachParams : function(formId){
			var sysId = this.params.sysid;
			return {sysId:sysId,formType:ATTACHMENT_FORMTYPE.FType_33,formId:formId};
		},
		/**
		 * 隐藏或显示控件
		 */
		...
		showOrHidFields : function(etype){
			var optionType = this.params.optionType;
			var cmpt_startEndDate = this.applyPanel.findFieldByName("cmpt_startEndDate");
			...
			switch(parseInt(etype)){
				case 1:{/*放款手续费*/
					...
					if(!exeItemsVal) rad_exeItems.setValue("2,3,4,5,6");/*默认选中所有的项目*/
					break;
				}case 3:{/*正常/逾期还款息费豁免*/
					cmpt_startEndDate.show();
					rad_exeItems.show();
					rad_exeItems.hideOrShowItems("1,6",true,true);
					if(!exeItemsVal) rad_exeItems.setValue("2,3,4,5");/*默认选中所有的项目*/
					break;
				}
			}
		},
		...
		showOrHidGrid : function(etype){
			var _this = this;
			if(this.currGrid) this.currGrid.hide();
			var contractId = this.applyPanel.getValueByName("contractId");
			var optionType = this.params.optionType || 1;
			var _params = {contractId : contractId,optionType:optionType,etype:etype};
			switch(parseInt(etype)){
				case 1:{/*放款手续费*/
					if(!this.freeGrid){
						this.createFreeGrid();
					...
					var valObj = this.applyPanel.getValueByName("cmpt_startEndDate");
					if(valObj) Ext.apply(_params,valObj);
					if(!this.planGrid){
						this.createPlanGrid();
		...
		showOrHidHeaders : function(){
			...
			var etype = this.applyPanel.getValueByName("etype");
			var itemMap = {
				'key_2':{cmn1:'rat',cmn2:'zinterest',show:false},/*豁免利息*/
				'key_3':{cmn1:'mat',cmn2:'zmgrAmount',show:false},/*豁免管理费*/
				'key_4':{cmn1:'pat',cmn2:'zpenAmount',show:false},/*豁免罚息*/
				'key_5':{cmn1:'dat',cmn2:'zdelAmount',show:false}/*豁免滞纳金*/
			}
			var flag = false;
			switch(parseInt(etype)){
				case 1:{/*放款手续费*/
					...
					Ext.apply(itemMap,{'key_6':{cmn1:'fat',cmn2:'zfreeamount',show:false}});/*豁免手续费*/
					flag = true;
					break;
				}case 3:{/*正常/逾期还款息费豁免*/
					flag = true;
			...
			if(showCmns1 && showCmns1.length >0){
				this.currGrid.setHeadersVisible(false,showCmns1.join(",")+","+showCmns2.join(","));
			}
		...
		/**
		 * 根据记录计算该记录指定列的和
		 * @param store Grid Store 对象
		 * @param showCmn1 本次豁免列
		 */
		sumTotalAmount : function(totalAmountCfg){
			...
			if(totalAmountCfg.fat){
				inforItemArr[inforItemArr.length] = "手续费："+totalAmountCfg.fat+"元";				
			}
			if(totalAmountCfg.rat){
				inforItemArr[inforItemArr.length] = "利息："+totalAmountCfg.rat+"元";				
			}
			if(totalAmountCfg.mat){
				inforItemArr[inforItemArr.length] = "管理费："+totalAmountCfg.mat+"元";				
			}
			if(totalAmountCfg.pat){
				inforItemArr[inforItemArr.length] = "罚息："+totalAmountCfg.pat+"元";				
			}
			if(totalAmountCfg.dat){
				inforItemArr[inforItemArr.length] = "滞纳金："+totalAmountCfg.dat+"元";				
			}
		...
		destroy : function(){
			if(null != this.appMainPanel){
				...
				if(this.currGrid) this.currGrid = null;
