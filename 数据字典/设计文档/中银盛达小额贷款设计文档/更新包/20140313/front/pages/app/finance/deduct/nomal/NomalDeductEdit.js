...
define(function(require, exports) {
	exports.WinEdit = {
		...
		changeTypeFstId : Ext.id(null,'changeTypeFstId_'),/*还款方式FieldSet 对象ID*/
		bussKey : null,/* 钥匙KEY */
		sysId : null,
		contractIds : null,
		uamount : null,
...
//						editable : false,
			...
			var txt_oldPaytype = FormUtil.getReadTxtField({fieldLabel:'当前还款方式',width:250});
			...
			var fst_chagePatype = FormUtil.createLayoutFieldSet({id:this.changeTypeFstId,
					title:'变更还款方式设置',checkboxToggle:true,collapsed : true,
					listeners : {
						expand:function(fst){
							_this.changePaytype(true);
						},
						collapse:function(fst){
							_this.changePaytype(false);
						}
					}
				},
				[{
					cmns : FormUtil.CMN_TWO,
					fields : [txt_oldPaytype,cbo_payType]
				}]);
		...
		/**
		 * 变更还款方式
		 * @param flag 布尔值 [true:变更还款方式,false ：不变更]
		 */
		changePaytype : function(flag){
			var fst_payType = Ext.getCmp(this.changeTypeFstId);
			var winHeight = this.appWin.getHeight();
			if(!winHeight) winHeight = 505;
			var fstHeight = 25;
			if(flag){
				winHeight += fstHeight;
			}else{
				...
				fst_payType.collapse(false);
			}
			this.appWin.setHeight(winHeight);
			var cbo_payType = this.appFrm.findFieldByName("payType");
			cbo_payType.reset();
			cbo_payType.setDisabled(!flag);
		},
		/**
		 * 扣收顺序改变时
		 */
...
//						}
		...
		callback : function(jsonData){
			...
			jsonData["appAmount"] = Cmw.getThousandths(appAmount)
					+ '元<br/><span style="color:red;">('+ Cmw.cmycurd(appAmount) + ')</span>';
			//还款状态
