...
define(function(require, exports) {
	exports.viewUI = {
		...
		getGotoUrl : function(url,_params){
			if(url == CUSTTAB_ID.flow_auditMainUITab.id ||
			url == CUSTTAB_ID.bussProcc_auditMainUITab.id){/*如果是跳流程审批面板*/
				var deskModParams = _params[DESK_MOD_TAG_QUERYPARAMS_KEY] || {};
				if(!deskModParams["applyId"]) deskModParams["applyId"] = deskModParams["id"];
				if(!_params["procId"]) _params["procId"] = null;
				var apptabtreewinId = _params["apptabtreewinId"];
				if(deskModParams) Ext.apply(_params,deskModParams);
				_params["sysId"] = _params["sysid"];
				_params["apptabtreewinId"] = apptabtreewinId;
				_params["isnewInstance"] = true;
			}
