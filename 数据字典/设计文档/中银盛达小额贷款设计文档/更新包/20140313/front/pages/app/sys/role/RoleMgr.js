...
Ext.extend(skythink.cmw.sys.RoleMgr,Ext.util.MyObservable,{
	...
	getToolBar : function(){
		...
		var barItems = [{
			...
			token : "取消权限",
			text : Btn_Cfgs.ROLE_CANCEL_BTN_TXT,
			iconCls:Btn_Cfgs.ROLE_CANCEL_BTN_CLS,
			tooltip:Btn_Cfgs.ROLE_CANCEL_TIP_BTN_TXT,
			handler : function(){
				self.globalMgr.cancelRoleRights(self);
			}
		},{type:"sp"},{
			token : "启用角色",
	...
	notify : function(data){
		...
		self.globalMgr.sysName = sysName;
		if(!self.globalMgr.sysId) self.globalMgr.sysId = sysId;
		...
		function reload(){
			...
			if(!curNode){
				self.apppanel.reload({sysid:sysId});
			}else{
				self.apppanel.reload({sysid:sysId},function(){
	...
	globalMgr : {
		 ...
		 * 取消角色权限
		 */
		cancelRoleRights : function(self){
			var roleId = self.apptree.getSelId();
			var roleName = self.apptree.getSelText();
			var curNode = self.apptree.curNode;
			var sysName = self.globalMgr.sysName;
			if(!roleId){
			...
			if(!checkIds || checkIds.length == 0){
				ExtUtil.alert({msg:'角色:['+roleName+']没有['+sysName+']的任何访问权限，不需要取消权限！'});
				return;
			}
			var sysId = self.globalMgr.sysId;
			var cfg = {params:{sysId : sysId,roleId:roleId},
			sfn:function(json_data){
				 self.apppanel.checkState = false;
				Cmw.unmask(self);
				Ext.tip.msg("提示","“"+role+"”的权限数据取消成功！");
				self.apppanel.reload({sysid:sysId},function(){
					self.globalMgr.checkRight(curNode,self);
				});
			},
			...
			ExtUtil.confirm({msg:'确定取消角色:['+roleName+']的访问权限?',fn : function(){
				if(arguments && arguments[0] != 'yes') return;
			 	Cmw.mask(self,"正在取消“"+role+"”的权限数据，请稍等...");
				EventManager.get('./sysRight_cancel.action',cfg);
			}});
		},
		/**
		 * 保存角色权限
