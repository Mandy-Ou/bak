...
Ext.extend(skythink.cmw.sys.PostMgr,Ext.util.MyObservable,{
	...
	getAppGrid : function(){
	  var structure = [
			...
			{header : '财务放款人员',name : 'isLoan',width:100,renderer : function(val){
				switch(val){
					case '1' : val = '是';break;
					case '2' : val = '否';break;
				}
	...
	//code,name,iconCls,type,url,orderNo
		...
		var isLoan = FormUtil.getRadioGroup({fieldLabel : '财务放款人员',name:'isLoan',width : 150,
		items : [{boxLabel : '是', name:'isLoan',inputValue:1},{boxLabel : '否', name:'isLoan',inputValue:2}]});
		...
		var appform = new Ext.ux.form.AppForm({
			...
			items : [hid_isenabled,id,code,name,isLoan,mnemonic,mtask,remark],
			width:200,
