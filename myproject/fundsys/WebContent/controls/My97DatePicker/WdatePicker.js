 ...
 */
 ...
 window.onload = function(){
 	 ...
 	 */
	var myYear //= document.getElementById("myYear").value = "";	//初始化时间
	var myMonth ///= document.getElementById("myMonth").value = "";	//初始化时间
	var myDay// = document.getElementById("myDay").value = "";
	var time ;
	$("#myYear").focus(function(){//输入焦点事件
		time = $("#myYear").val();
		myYear = time.split("-")[0]
		myMonth = time.split("-")[1];
		myDay = time.split("-")[2];	
		$("#time").val(time);
		$("#myYear").val(time);
		$("#myMonth").val(myMonth);
