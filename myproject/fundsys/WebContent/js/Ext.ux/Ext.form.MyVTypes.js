...
Ext.form.MyVTypes =  Ext.apply(Ext.form.VTypes, {
	//QQ或MSN
	'qq' :function(_v){
		var flag = false;
		  if(/^[1-9][0-9]{4,}$/.test(_v)){
		  	flag = true;
		  }else{
		  	if(/^(\w+)([\-+.][\w]+)*@(\w[\-\w]*\.){1,5}([A-Za-z]){2,6}$/.test(_v)){
		  		flag = true;
		  	}
		  }
		  return flag;
	},
	"qqText" :  RegExpText.qqText,
	"qqTextMask" :  /[0-9]/i,
	  // 年龄
	"age" : function(_v) {
	   if (/^\d+$/.test(_v)) {
	...
	"repasswordText" : RegExpText.repasswordText,
	"repasswordMask" : /[a-z0-9]/i,
	// 邮政编码
	"postcode" : function(_v) {
	   return /^[1-9]\d{5}$/.test(_v);
	},
	"postcodeText" : RegExpText.postcodeText,
	"postcodeMask" : /[0-9]/i,
	// IP地址验证
	"ip" : function(_v) {
	...
	"telephoneText" : RegExpText.telephoneText,
	"telephoneMask" : /[0-9\-]/i,
	// 手机
	"mobile" : function(_v) {
	   return /^(1(([35][0-9])|(47)|[8][01236789]))\d{8}$/.test(_v);
	},
	"mobileText" : RegExpText.mobileText,
	"mobileMask" : /[0-9]/i,
	// 身份证
	"IDCard" : function(_v) {
	   ...
	   if (area[parseInt(_v.substr(0, 2))] == null) {
	    this.IDCardText = RegExpText.IDCardText_1;
	    return false;
	   }
	   // 身份号码位数及格式检验
	   switch (_v.length) {
	    case 15 :
	     ...
	     if (ereg.test(_v))
	      ...
	      this.IDCardText = RegExpText.IDCardText_2;
	      return false;
	     }
	     break;
	    case 18 :
	     ...
	     if (ereg.test(_v)) {// 测试出生日期的合法性
	      ...
	      if (M == idcard_array[17]) {
	       ...
	       this.IDCardText = RegExpText.IDCardText_3;
	       return false;
	      }
	     } else {
	      this.IDCardText = RegExpText.IDCardText_4;
	      return false;
	     }
	     break;
	    default :
	     this.IDCardText = RegExpText.IDCardText_5;
	     return false;
	     break;
	   }
	},
	"IDCardText" : RegExpText.IDCardText_6,
	"IDCardMask" : /[0-9xX]/i
});