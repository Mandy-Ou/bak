 ...
 */ 
cmw.skythink.BackReceiptMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(cmw.skythink.BackReceiptMgr,Ext.util.MyObservable,{
	initModule : function(tab,params){
		this.module = new Cmw.app.widget.AbsGContainerView({
			querytitle : '回款收条',//重写主题
			tab : tab,
			params : params,
			hasTopSys : false,
	...
	getQueryFrm : function(){
		...
		var wd = 200;
		//1.客户姓名
		var txt_name = FormUtil.getTxtField({fieldLabel : '客户姓名',name:'name',width : wd , maxLength : 30});
		/**
		...
		/**
		 * 3.回款日期     计算时间 txt_outDate、txt_endDate、txt_outAndEndDate
		 */
		var txt_outDate = FormUtil.getDateField({name : 'outDate', width : 88});//回款日期起始时间
		var txt_endDate = FormUtil.getDateField({name : 'endDate' , width : 88});//回款日期结束时间
		...
		//4.票号
		var txt_rnum = FormUtil.getTxtField({fieldLabel : '票号',name:'rnum',width:wd,maxLength : 80});
		//5.收款人账户名
		...
		var layout_fields = [{cmns:'THREE',fields:[txt_name,txt_amount,txt_outAndEndDate,txt_rnum,txt_rtacname,txt_rtaccount]}]
		...
		var barItems = [{
			...
			handler : function(){
				var id = self.appgrid.getSelId();
					if (!id)
						return;
				self.globalMgr.winEdit.backReceiptDetail({key : this.token,self : self,_appgrid : self.globalMgr._appgrid,orgtype : 3 ,id:id});
			}
		}];
	...
	getAppGrid : function(){
		var self = this;
			...
			{header: '收条发送人',name: 'reman'},
			{header: '汇票数量',name: 'rcount'},
			{header: '出票人账号',name: 'pbank'},
			{header: '付款行',name: 'rcount'},
			...
			{header: '回款收条签收日期',name: 'recetDate',
				renderer: function(val) {
					switch (val) {
		        		case "":
		          	 		val = "暂无";
	...
	globalMgr : {
		...
		winEdit : {
			 ...
			 * 查看详情
			 */
			backReceiptDetail : function(parentCfg){
				...
				if(self.appCmpts[winkey]){
					...
					switch(winkey){
						case '查看详情' : {
							winModule = "BackReceiptDetail";
							break;
						}
					}
					Cmw.importPackage('pages/app/funds/receiptbackpayment/' + winModule,function(module) {
