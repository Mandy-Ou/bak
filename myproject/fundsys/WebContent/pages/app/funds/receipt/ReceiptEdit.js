...
define(function(require, exports) {
	exports.viewUI = {
		...
		createAppWindow : function(){
			...
			this.iframeId = Ext.id(null,'receiptEdit');  
...
//			var htmlArr = ['<iframe id="'+this.iframeId+'" name="'+this.iframeId+'"  style="width:100%; height:100% " frameborder="0" src="pages/app/funds/receipt/ReceiptEdit.jsp'+this.globalMgr.params+'"></iframe>'];
			var iframeHtml = htmlArr.join(" ");
			this.appWin = new Ext.ux.window.MyWindow({title : "汇票收条管理",width:750,height:600,autoScroll:true,draggable:true,maximizable:true,minimizable:true,tbar:this.tbar,html :iframeHtml});
		},
		getToolBar : function(){
			var _this = this;
			var toolBar = null;
		...
		print : function(){
			...
			//PrintManager.print(iframeEle.document.body.innerHTML);
		//	iframeEle.PrintMgr.print();
		//	iframeEle.AuditProjectMgr.doPrint();
//			this.iframeId.window.print();
//			this.iframeId.document.execCommand('print');
		},
		/**
		 * 显示弹出窗口
		...
		show : function(_parentCfg){
			...
			if(_parentCfg) this.setParentCfg(_parentCfg);
				...
				this.globalMgr.params = "?id=" +id+"&orgtype="+ orgtype;
				//var parentContainerId = this.appWin.getId();
				//this.globalMgr.url = this.globalMgr.url;//+"&parentContainerId="+parentContainerId;
				//_this.loadData(_parentCfg.orgtype,_parentCfg.id);
				this.createAppWindow();
				this.appWin.show();
			}*/
		},
...
//				var url = iframeEle.dom.src;
		...
		makeDatas : function(){
			...
			/*添加*/
				...
				var recetDate = document.getElementById("time").value;
	...
	//			var form = iframeEle.document.forms[0];
		...
		globalMgr : {
			params : null,
			url : 'pages/app/funds/receipt/ReceiptEdit.jsp',
			id : null,
			orgtype: null
		},
