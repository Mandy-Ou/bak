 ...
 */ 
cmw.skythink.ReceiptApproval = function(){
	this.init(arguments[0]);
}

Ext.extend(cmw.skythink.ReceiptApproval,Ext.util.MyObservable,{
	initModule : function(tab,params){
		this.module = new Cmw.app.widget.AbsGContainerView({
			querytitle : '汇票收条审批',//重写主题
			tab : tab,
			params : params,
			hasTopSys : false,
	...
	getAppGrid : function(){
		var self = this;
			...
			var appgrid = new Ext.ux.grid.AppGrid({
			    ...
			    url: './fuReceipt_auditlist.action',
			    needPage: true,
			    isLoad: true,
			    autoScroll:true,
			    keyField: 'id'
	...
	globalMgr : {
		...
		submitApplyForm : function(_this){
			...
			if(!applyId) return;
			var codeObj = _this.appgrid.getCmnVals("rnum");
			var sysId = _this.params.sysid;
			var menuId = _this.params.nodeId;
//			var contractId = codeObj.contractId;
			 ExtUtil.confirm({title:'提示',msg:'确定要审批提交票号为："'+codeObj.rnum+'"的汇票收条申请单?',fn:function(){
		 	  		if(arguments && arguments[0] != 'yes') return;
					var params = {isnewInstance:true,sysId:sysId,applyId:applyId,menuId:menuId,procId:null,bussProccCode:'B5222'};
