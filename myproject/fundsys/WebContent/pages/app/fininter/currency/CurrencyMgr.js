...
/**
 * 币别配置管理 UI smartplatform_auto 2013-03-29 15:38:16
 */ 
skythink.cmw.fininter.CurrencyMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(skythink.cmw.fininter.CurrencyMgr,Ext.util.MyObservable,{
	...
	getQueryFrm : function(){
		var self =this;
		var txt_code= FormUtil.getTxtField({fieldLabel : '币别代码',name:'code'});
		var txt_name = FormUtil.getTxtField({fieldLabel : '币别名称',name:'name'});
	...
	getAppGrid : function(){
	    ...
	    var structure = [{header : '可用标识',name : 'isenabled',width:60,renderer : Render_dataSource.isenabledRender},
						{header:'币别代码' ,name:'code' ,width:100},
						{header:'币别名称' ,name:'name' ,width:100},
						{header:'汇率' ,name:'erate' ,width:100},
						{header:'小数位数' ,name:'fscale' ,width:100},
						{header:'财务币别ID' ,name:'refId' ,width:100},
						{header:'同步日期' ,name:'createTime' ,width:120}
						];
		var appgrid = new Ext.ux.grid.AppGrid({
			title : this.gridTitle,
			tbar : this.toolBar,
			structure : structure,
			url : './fsCurrency_list.action',
			keyField : 'id',
			needPage : true,
			isLoad: false
		});
		...
		synchronousData : function(_this){
			 ...
			 fn:function(btn){
			 	if(btn != 'yes') return;
					...
					EventManager.get('./fsCurrency_synchronous.action',{params:{finsysId:_this.globalMgr.finsyscfgId},
