...
define(function(require, exports) {
	exports.WinEdit = {
		...
		saveData : function(){
			var _this = exports.WinEdit;
			var datasource = _this.appFrm.getValueByName("datasource");
			var controlType = _this.appFrm.getValueByName("controlType");
			if(controlType==5 || controlType==7 || controlType==8 || controlType==9){
				if(!datasource){
					ExtUtil.alert({msg:"配置的数据源不为空！"});
					return;
				}
				if(datasource.indexOf(",")==-1){
					ExtUtil.alert({msg:"配置的数据源格式有误！"});
					return;
				}
			}
			
			EventManager.frm_save(_this.appFrm,{sfn:function(json_Data){
				if(_this.refresh) _this.refresh(json_Data);
					_this.appFrm.reset();
					_this.appWin.hide();
			}});
		},
		...
		createForm : function(){
												...
												listeners:{
													'select' : function(combo,record,index ){
														if(index==5 || index==7 || index==8 || index==9){
															if(txt_datasource.allowBlank ){
																txt_datasource.allowBlank = false;
																var fieldLabel = txt_datasource.fieldLabel;
																Ext.get("datasourceField").update(FormUtil.REQUIREDHTML+txt_datasource.fieldLabel);
															}
														}else{
															txt_datasource.allowBlank = true;
															var fieldLabel = txt_datasource.fieldLabel;
															Ext.get("datasourceField").update('<span id="datasourceField">数据源</span>');
															txt_datasource.clearInvalid(); 
														}
													}
												}
												});
			/*数据源*/
			var txt_datasource = FormUtil.getTxtField({fieldLabel : '<span id="datasourceField">数据源</span>',name:'datasource',width:370,maxLength:200});
			/*是否必填*/
			...
			/*所在列*/	
			var txt_col = FormUtil.getIntegerField({fieldLabel : '所在列',name:'col',allowBlank:false,emptyText:"最大列不能大于3",listeners:{
				'change': function(txt,newValue,oldValue){
					if(newValue>3 || newValue< 1){
						ExtUtil.alert({msg:"填写的所在列超出了,自定义表单范围！"});
						return;
					}
				}
			}});
