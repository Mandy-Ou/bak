...
Ext.extend(skythink.cmw.workflow.bussforms.PledgeMgr,Ext.util.MyObservable,{
	...
	notify : function(data){
		...
		this.globalMgr.appgrid.reload({gtype:this.globalMgr.gvlistid,formId:this.globalMgr.formId,state:"0,1"});//点击系统图标时候加载树
	},
	getToolBar : function(){
	...
	refresh:function(optionType,data){
		...
		var _this = this;
		this.globalMgr.appgrid.reload({gtype:this.globalMgr.gvlistid,formId: this.globalMgr.formId,state:"0,1"},function(){
			switch(optionType){
	...
	globalMgr : {
		...
		createAttachMentFs : function(_this){
			 ...
			 *  ---- */
			var attachMentFs = new  Ext.ux.AppAttachmentFs({title:'质押物附件列表',isLoad:false,dir : 'pledge_path',isSave:true,isNotDisenbaled:true});
			return attachMentFs;
		},
		/**
		 * 表格单击事件
	...
	/**
		...
		var barItems = [{
			token :"落实确认", 
			text : Btn_Cfgs.IMPLEMENT_BTN_TXT,
			iconCls:Btn_Cfgs.IMPLEMENT_CLS,
			tooltip:Btn_Cfgs.IMPLEMENT_TIP_BTN_TXT,
			handler : function(){
				self.globalMgr.winEdit.show({key:"落实确认",self:self});
			}
		}];
		...
		winEdit : {
			show : function(parentCfg){
				...
				var sel=parent.getSelRow();
				parentCfg.code=sel.get("code");
				winModule="PledgeImpEdit";
				var selId=parent.getSelId();
				if(!selId)return;
				if(sel.get("state")=="1"){
					ExtUtil.alert({msg:'该质押物已落实！'});
					return;
				}
