...
Ext.extend(skythink.cmw.workflow.bussforms.MortgageMgr,Ext.util.MyObservable,{
	...
	notify : function(data){
		...
		this.globalMgr.appgrid.reload({gtype:this.globalMgr.gvlistid,formId:this.globalMgr.formId,state:"0,1"});
	},
	getToolBar : function(){
...
//		detailpanel.doLayout();
	...
	refresh:function(optionType,data){
		...
		var _this = this;
		this.globalMgr.appgrid.reload({gtype:this.globalMgr.gvlistid,formId:formId,state:"0,1"},function(){
			switch(optionType){
	...
	winEdit : {
			show : function(parentCfg){
				...
				winModule="MortgageImpEdit";
				var selId=parent.getSelId();
				if(!selId)return;
				if(sel.get("state")=="1"){
					ExtUtil.alert({msg:'该抵押物已落实！'});
					return;
				}
					Cmw.importPackage('pages/app/workflow/variety/formUIs/implement/'+winModule,function(module) {
