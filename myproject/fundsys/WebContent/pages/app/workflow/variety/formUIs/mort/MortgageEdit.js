...
define(function(require, exports) {
	exports.WinEdit = {
		...
		createAppWindow : function(){
			...
			this.appWin = new Ext.ux.window.AbsEditWindow({autoScroll : true,closeAction:'close',width:650,getUrls:this.getUrls,appFrm : this.appFrm,
			optionType : this.optionType, refresh : this.refresh,hidden : true,isNotSetVs : true,eventMgr:{saveData:this.saveData},prevNextBtnHiden:true
			});
		},
		/**
		 * 显示弹出窗口
...
//			}
		...
		createForm : function(){
			...
			var layout_fields = [txt_id,txt_formId,txt_gtype,{
			    ...
			    fields: [txt_code,txt_name,quan_unint, txt_oldVal, txt_mpVal, txt_mman, txt_conTel, bdat_morTime]
		...
		saveData : function(){
			...
			if(!quantity || quantity<=0){
				ExtUtil.alert({msg:"输入的抵押物数量不能为空且必须大于0！"});
				return;
			}
			if(!unint){
				ExtUtil.alert({msg:"输入的抵押物单位不能为空！"});
				return;
			}
			if(oldVal<=0){
			...
			if(mpVal<=0){
				ExtUtil.alert({msg:"输入的抵押物金额必须大于0！"});
				return;
			}
