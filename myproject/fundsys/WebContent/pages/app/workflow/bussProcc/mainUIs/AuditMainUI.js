	...
	exports.viewUI = {
		...
		getMainUI : function(tab,params){
			...
			tab.notify = function(_tab){
				...
				_this.applyDetailMod.show(_this.params);
			}
		...
		setParams : function(tab,params){
			this.tab = tab;
			this.params = params;
//			this.params.bussProccCode = 'B0004';
		},
		/**
		 * 创建组件
		 */
		createCmpts : function(){
			var height = CLIENTHEIGHT - 205;
			this.topPanel = new Ext.Panel({border:false,id:ApplyActivePnlId.topActiveRecipt});
//			var menuLabelId = Ext.id(null,'bussMakeMenuId');/*菜单事项总提示ID*/
//			this.auditPanel = new Ext.Panel({title :'审批信息<span id="'+menuLabelId+'" class="buss_menu_list_menuLabel">XXXXXX</span>',height:500,autoScroll:true,menuLabelId:menuLabelId});
			this.auditPanel = new Ext.Panel({title :'审批信息',height:height,autoScroll:true});
			this.bussMakePanel = new Ext.Panel({title :'业务办理',height:height,hidden:true,autoScroll:true});
			this.processImgPanel = new Ext.Panel({title :'流程预览',height:height,autoScroll:true});
			this.tabPanel = new Ext.TabPanel({
				activeItem:0,autoScroll:true,border:false,
			...
			this.tabPanel.addListener('render',function(tabPnl){
				...
				var tabHeight = _this.tab.getHeight()-Ext.get(ApplyActivePnlId.topActiveRecipt).getHeight();
				tabPnl.setHeight(tabHeight);
				tabPnl.doLayout();
			});
			this.appMainPanel = new Ext.Panel({border:false,items:[this.topPanel,this.tabPanel],autoScroll:true});
		},
		/**
		 * 组装控件
		 */
		...
		loadAuditMgrModInfo : function(){
			if(!this.auditMgrMod){
				...
				Cmw.importPackage('/pages/app/workflow/bussProcc/mainUIs/AuditListMod',function(module) {
		...
		loadBussMgrModInfo : function(){
			...
			if(!this.bussMgrMod){
				...
				Cmw.importPackage('pages/app/workflow/bussProcc/mainUIs/BussMenuListMod',function(module) {
		...
		loadProcessImgModInfo : function(){
			if(!this.processImgMod){
				...
				Cmw.importPackage('/pages/app/workflow/bussProcc/mainUIs/ProcessImgMod',function(module) {
		...
		isHideAuditForm : function(bussFormDatas){
			...
			bussFormDatas =  Ext.decode(bussFormDatas);
		...
		resize : function(adjWidth,adjHeight){
			...
			if(adjHeight < height) adjHeight = height;
