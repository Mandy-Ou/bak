...
<table cellspacing="0" cellpadding="0">
  ...
  <tr>
   ...
   <td colspan="4" width="297" style="nowrap="nowrap"><span style="word-break:keep-all;">评审会时间：<input style="width:100px;" class="Wdate" type="text" id="auditDate" name="data" onClick="WdatePicker()"></span>
    <span id="auditDates"><%=auditDate%></span> 
    </td>
    <td colspan="3">评审会地点：<input type="text" id="address" /><span  id="addresss"><%=address%></span></td>
  </tr>
  <tr>
  <td colspan="4"><label for="R3_CN3"> 主持人：</label><input type="text" name="address2"  id="address2" onClick="chanClick();"/>
  <span id="hostMans"><%=hostMan %></span> </td>
    <td colspan="3">记录人:<input type="text" id="recordMan" /><span id="recordMans"><%=recordMan %></span> </td>
  </tr>
  <tr>
...
<!--主持人控件显示  -->
 <div id="selectItem2" class="selectItemhidden" style="display: none;"> 
	<div id="selectItemAd" class="selectItemtit bgc_ccc"> 
		<h2 id="selectItemTitle" class="selectItemleft">请选择主持人</h2> 
...
window.onload=function(){ 
	/*只控制textarea的特殊元素  */
	document.getElementById("breedsd").disabled=true;
...
function chanClick(){  
    $.ajax({  
            ...
            /* 	console.log(data);
            	alert(data.list[0].id); */
            		var subs=data.list;//获取data中list的数组
            		var bb =$(document).find("#selectSub");  
       		  for ( var i = 0; i < subs.length; i++) {  
        	  	 bb.append("<input type='checkbox' id='"+subs[i].userId+"' name='cr' value='"+subs[i].userName+"'>"+subs[i].userName+"</input>"); 
       		  }  
            }  
        });  
}; 
