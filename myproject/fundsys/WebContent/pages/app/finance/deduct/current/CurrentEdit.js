	...
	exports.WinEdit = {
		...
		createAppDetailPn : function(){
			...
			var htmlArrs_1 = [
					...
					'<tr>',
					'<tr><th colspan="6" style="font-weight:bold;text-align:left;" >&nbsp;&nbsp;累计未收金额>>></th><tr>',
					'<tr>',
						'<th col="zprincipal">本金</th> <td col="zprincipal" id="'+idPrefix+'zprincipal">&nbsp;</td>',
					...
					'<tr>',
						...
						'<th col="ztotalAmount">应收合计</th> <td col="ztotalAmount" id="'+idPrefix+'ztotalAmount">&nbsp;</td>',
					'<tr>'
					];
...
//			rad_order.addListener('change',function(radgroup,checked){_this.orderChangeListener(radgroup,checked);});
			...
			var num_principal = FormUtil.getMoneyField({
				fieldLabel : '本金',
				name : 'principal',
				allowBlank : false,
				width : 100,
			...
			var num_fat = FormUtil.getMoneyField({
				fieldLabel : '随借随还管理费',
				name : 'fat',
				allowBlank : false,
				width : 100,
...
//			var cbo_mgrOrders = FormUtil.getLCboField({hideLabel :true,name:'mgrOrders',
//			data:[["1","优先扣随借随还管理费"],["2","最高优先级费用项扣完后，紧接着扣“随借随还管理费”费用项"]],allowBlank : false,value:1,width:350});
//			var highSet = FormUtil.getFieldSet({title:'高级设置',items:[{xtype:'label',html :'<span style="margin-bottom:5px;">随借随还管理费优先级设置:</span>',width:250},cbo_mgrOrders]});
			var layout_fields = [txt_contractId,txt_zprincipal,txt_zinterest,txt_zmgrAmount,txt_zpenAmount,txt_zdelAmount,txt_ztotalAmount,{
						cmns : FormUtil.CMN_THREE,
						fields : [cbog_accountId, txt_bankAccount,
								dat_rectDate/*,rad_order*/,num_principal,num_interest,num_mgrAmount,num_penAmount,num_delAmount,num_fat,num_tat,num_oddAmount]
					}]//,highSet];
				
			var frm_cfg = {
				title : '本次收款信息录入<span style="float:right;color:red;" id="'+_this.display_dateSpanId+'"></span>',
				labelWidth : 95,
				url : './fcCurrent_receivables.action'
			};
		...
		collectAmounts : function(){
			...
			var zvals = this.appFrm.getValuesByNames("zprincipal,zinterest,zmgrAmount,zpenAmount,zdelAmount");
			var cvals = this.appFrm.getValuesByNames("principal,interest,mgrAmount,penAmount,delAmount,fat");
			//---> 计算应收
			var fat = cvals.fat || 0;
			var ztotalAmount = 0;
		...
		callback : function(jsonData){
			...
			jsonData["payType"] = Render_dataSource.payTypeRender(jsonData["payType"])+'<br/><span style="color:red;">(利率类型:'+Render_dataSource.rateTypeRender(jsonData["rateType"])+'&nbsp;'+jsonData["rate"]+'%)</span>';
			var array = ['mrate','prate','urate','frate'];
			for (var index = 0; index < array.length; index++) {
				jsonData[array[index]] = jsonData[array[index]] +'%';
			}
			var amountArr = ["zprincipal","zinterest","zmgrAmount","zpenAmount","zdelAmount","ztotalAmount"];
		...
		setValueData : function(jsonData){
			...
			var keys = ["zprincipal","zinterest","zmgrAmount","zpenAmount","zdelAmount","ztotalAmount"];
			var fieldNames = ["principal","interest","mgrAmount","penAmount","delAmount","tat"];
			var formDatas = {};
		...
		checkDate : function(rectDate){
			...
			if(this.payDate && Ext.isString(this.payDate)){
				...
				if(rectDate_time < payDate_time){
					...
					emsg = "收款日期不得小于"+payDateStr+"(即：不能小于放款日期)!";
			...
			if(this.lastDate && Ext.isString(this.lastDate)){
				...
				if(rectDate_time < lastDate_time){
					...
					emsg = "收款日期不得小于"+lastDateStr+"(即：不能小于上次收款日期)!";
					ExtUtil.alert({msg:emsg});
					this.errArr[this.errArr.length] = emsg;
					return false;
				}
		...
		setCalculateResult : function(json_data){
			...
			var t_iamount = json_data.t_iamount;
			var t_mamount = json_data.t_mamount;
			...
			if(lateDatas && lateDatas.length > 0){
				...
				for(var i=0,count=lateDatas.length; i<count; i++){
					...
					var penAmount = amountData.penAmount;
					var delAmount = amountData.delAmount;
					var ypenAmount = amountData.ypenAmount;
					var ydelAmount = amountData.ydelAmount;
			...
			if(zprincipal > 0) zprincipal = parseFloat(zprincipal);
