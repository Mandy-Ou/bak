...
Ext.extend(cmw.skythink.AccountMgr,Ext.util.MyObservable,{
	...
	getToolBar : function(){
		...
		,{type:"sp"},{
			...
			handler : function(){
				EventManager.enabledData('./fcAccount_enabled.action',{type:'grid',cmpt:self.appgrid,optionType:OPTION_TYPE.ENABLED,self:self});
			}
		},{
			token : '禁用',
			...
			handler : function(){
				EventManager.disabledData('./fcAccount_disabled.action',{type:'grid',cmpt:self.appgrid,optionType:OPTION_TYPE.DISABLED,self:self});
			}
		},{
			token : '删除',
			...
			handler : function(){
				EventManager.deleteData('./fcAccount_delete.action',{type:'grid',cmpt:self.appgrid,optionType:OPTION_TYPE.DEL,self:self});
			}
		}];
		...
		winEdit : {
			show : function(parentCfg){
				...
				if(_this.appCmpts[winkey]){
					...
					Cmw.importPackage('pages/app/finance/fcinit/account/'+winModule,function(module) {
			...
			var parentCfg = {
				...
				callback : function(selIds,record){
					...
					var code = record.get("code");
					 EventManager.get('./fcAccount_update.action',{params:{id:id,fsbankAccountId:selIds,refId:refId,code:code},sfn:function(json_data){
					 	_this.globalMgr.query(_this);
