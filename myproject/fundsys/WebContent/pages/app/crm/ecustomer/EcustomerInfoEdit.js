 ...
 * 卡片菜单管理
 */
skythink.cmw.crm.EcustomerInfoEdit = function(){
	this.init(arguments[0]);
}

/*-----------继承Observable的事件方法--------*/
Ext.extend(skythink.cmw.crm.EcustomerInfoEdit,Ext.util.MyObservable,{
	...
	refresh:function(tab,params){
		...
		var ecustomerInfoId = params.ecustomerId;
		var code = params.code;
		var appgrid = params.appgrid;
		if(baseId){	/*修改 */
			var registerTime = Ext.util.Format.date(params.registerTime,'Y-m-d');
			var parentCfg = {parent : this.appPanel,appgrid : appgrid,
			optionType : OPTION_TYPE.EDIT,params : {baseId:baseId,id : ecustomerInfoId,sysId :params.sysid,code:params.code,
			name : params.name,serialNum:params.serialNum,tradNumber:params.tradNumber,orgcode:params.orgcode,
			registerTime:registerTime,regman:params.regman}};
