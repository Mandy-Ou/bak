	...
	private String getGuaCustomet(final ApplyEntity entity)
		...
		if(StringHandler.isValidStr(guaId)){
			...
			case SysConstant.CUSTTYPE_1:
				List<CustomerInfoEntity> customerInfoEntityList = customerInfoService.getEntityList(params);
				if(!customerInfoEntityList.isEmpty() && customerInfoEntityList.size()>0){
					for(CustomerInfoEntity customerInfoEntit: customerInfoEntityList){
						String name = customerInfoEntit.getName();
						sbName.append(name+",");
					}
