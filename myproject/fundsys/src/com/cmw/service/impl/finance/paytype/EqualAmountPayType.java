 ...
 *
 */
public class EqualAmountPayType extends PayTypeCalculateAbs {
	protected Map<String,Object> formulaParams = new HashMap<String, Object>();
	private double selfAmount = 0d; /*当期还款本金*/
	@Override
	...
	private void doPlanAmounts() throws ServiceException{
		...
		this.selfAmount = cacluteSelfAmount();
		//只有一期的情况,按最后一期公式计算
	...
	private double cacluteSelfAmount() throws ServiceException {
		double selfamount;
		formulaParams.put("appAmount", payAmount);
		int totalPhases = phasesMap.size();
		if(loanContractEntity.getIsadvance().intValue() == BussStateConstant.ISADVANCE_1) totalPhases -= 1;
		formulaParams.put("totalPhases", totalPhases);
		selfamount = getRateAmount(formulaParams, FormulaType.AMOUNT_FORMULA);
		selfamount = StringHandler.Round(selfamount, 2);
		return selfamount;
	}
	...
	private double getCenterPlanAmounts(ChildPlanEntity entity) throws ServiceException {
		...
		for(int i=2; i< phases; i++){
			...
			double selfamount = this.selfAmount;
