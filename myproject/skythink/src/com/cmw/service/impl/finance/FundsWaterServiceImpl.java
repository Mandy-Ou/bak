...
public class FundsWaterServiceImpl extends AbsService<FundsWaterEntity, Long> implements  FundsWaterService {
	...
	public <K, V> void calculate(Map<AmountLogEntity, DataTable> logDataMap,UserEntity user)
		...
		for(AmountLogEntity amountLog : logKeys){
			...
			this.getMoney(amountLog,cloneFundsWate,ownfundsId,false);
	 ...
	 */
	@Override
	public void calculate(Map<AmountLogEntity, DataTable> logDataMap,UserEntity user, Boolean isOut)throws ServiceException {
		if(null == logDataMap || logDataMap.size() == 0) return;
		FundsWaterEntity fundsWater = new FundsWaterEntity();
		BeanUtil.setCreateInfo(user,fundsWater);
		Set<AmountLogEntity> logKeys = logDataMap.keySet();
		...
		for(AmountLogEntity amountLog : logKeys){
			FundsWaterEntity cloneFundsWate= (FundsWaterEntity) fundsWater.clone();
			this.getMoney(amountLog,cloneFundsWate,ownfundsId,isOut);
			fundsWaterList.add( cloneFundsWate);
			BigDecimal amounts = cloneFundsWate.getAmounts();
			totalAmount = BigDecimalHandler.add2BigDecimal(totalAmount, amounts);
		}
	...
	private void calculateOwnFundsTotalAmount(OwnFundsEntity ownFundsObj,AmountLogEntity firstAmountLog, BigDecimal ytotalAmount){
		...
		Integer bussTag = firstAmountLog.getBussTag();
		...
		switch (category) {
			...
			case BussStateConstant.AMOUNTLOG_CATEGORY_7:
			case BussStateConstant.AMOUNTLOG_CATEGORY_8:{
				if(null != bussTag && (bussTag.intValue() == BussStateConstant.AMOUNTLOG_BUSSTAG_13)){//息费返还做减法操作
					totalAmount = BigDecimalHandler.sub2BigDecimal(totalAmount, ytotalAmount,2);
					uamount = BigDecimalHandler.sub2BigDecimal(uamount, ytotalAmount,2);
				}else{
	 ...
	 * @throws ServiceException 
	 */
	private void getMoney(AmountLogEntity amountLog,FundsWaterEntity cloneFundsWate, Long ownfundsId, Boolean isOut) throws ServiceException {
		...
		if(null != isOut && isOut.booleanValue()){
			...
			if(null != amount_BussTag && amount_BussTag.intValue() == BussStateConstant.AMOUNTLOG_BUSSTAG_13){/*当是息费返还时，当支出处理*/
				waterType = BussStateConstant.FUNDSWATER_WATERTYPE_1;
			}
		}
	...
	private Integer getWaterBussTag(Integer amount_bussTag){
		...
		case BussStateConstant.AMOUNTLOG_BUSSTAG_12:
			...
			break;
		case BussStateConstant.AMOUNTLOG_BUSSTAG_13:
			bussTag = BussStateConstant.FUNDSWATER_BUSSTAG_8;
			break;
		default:
			break;
		}
