...
//		int lastDay = DateUtil.getMonthTotalDays(paydate);
		Date newEndDate = DateUtil.addDaysToDate(paydate, 1);
		int prevPayDay = DateUtil.getDays(eqDate);
		int xpayDay = DateUtil.getDays(newEndDate);
		boolean result = (prevPayDay == xpayDay);
		Integer setdayType = loanContractEntity.getSetdayType();
		if( (StringHandler.isValidObj(setdayType) &&  setdayType == BussStateConstant.LOANCONTRACT_SETDAYTYPE_1) &&
				result){  /*当上一期的结息日的号数与最后一期结息日的号数相等或少一天时，按月算息*/
