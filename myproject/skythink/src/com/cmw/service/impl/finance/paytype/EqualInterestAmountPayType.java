 ...
 *
 */
public class EqualInterestAmountPayType extends PayTypeCalculateAbs {
	...
	private ChildPlanEntity getFirstPlanAmount() throws ServiceException {
		...
		if(loanContractEntity.getIsadvance().intValue() == BussStateConstant.ISADVANCE_0){/*非预收息*/
			formulaParams.put("rate", getRate(RateType.MONTH_RATE, false));
			formulaParams.put("mrate", getRate(RateType.MONTH_RATE, true));
			int zphases = phasesMap.size();
			formulaParams.put("zphases", zphases);
			xamount = getRateAmount(formulaParams, FormulaType.TOTALAMOUNT_FORMULA);
			formulaParams.put("totalAmount", xamount);
			formulaParams.put("interest", rateamount);
			formulaParams.put("mgrAmount", mgramount);
			selfamount = getRateAmount(formulaParams, FormulaType.AMOUNT_FORMULA);
		}
	...
	private double getCenterPlanAmounts(ChildPlanEntity entity) throws ServiceException {
		...
		for(int i=2; i< phases; i++){
			...
			formulaParams.put("reprincipal", bamount);
			...
			zphases--;
			formulaParams.put("zphases", zphases);
			double xamount = getRateAmount(formulaParams, FormulaType.TOTALAMOUNT_FORMULA);
			formulaParams.put("totalAmount", xamount);
			formulaParams.put("interest", rateamount);
			formulaParams.put("mgrAmount", mgramount);
			double selfamount = getRateAmount(formulaParams, FormulaType.AMOUNT_FORMULA);
	...
	private ChildPlanEntity getLastPlanAmount(double prebamount) throws ServiceException {
		...
		formulaParams.put("rate", getRate(RateType.MONTH_RATE, false));
		formulaParams.put("mrate", getRate(RateType.MONTH_RATE, true));
		formulaParams.put("totalPhases", 1);
		formulaParams.put("zphases", 1);
		double xamount = getRateAmount(formulaParams, FormulaType.TOTALAMOUNT_FORMULA);
		formulaParams.put("totalAmount", xamount);
		formulaParams.put("interest", rateamount);
		formulaParams.put("mgrAmount", mgramount);
	...
	private void doOnlyLast() throws ServiceException {
		...
		formulaParams.put("rate", getRate(RateType.MONTH_RATE, false));
		formulaParams.put("mrate", getRate(RateType.MONTH_RATE, true));
		formulaParams.put("totalPhases", phases);
		formulaParams.put("zphases", phases);
		double xamount = getRateAmount(formulaParams, FormulaType.TOTALAMOUNT_FORMULA);
		formulaParams.put("totalAmount", xamount);
		formulaParams.put("interest", rateamount);
		formulaParams.put("mgrAmount", mgramount);
