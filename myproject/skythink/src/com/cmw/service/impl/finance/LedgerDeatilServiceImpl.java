...
//		params.put("state", SqlUtil.LOGIC_GTEQ+SqlUtil.LOGIC+BussStateConstant.FIN_APPLY_STATE_3);
	...
	private DataTable serviceHandler(Long sysId,DataTable applyDt, DataTable loanDt,List<CustomerInfoEntity> customerList,
		...
		String columnNames = "id,applyCode,code,name,cardNum,patentNumber,address," +
				"bankOrg,account,ecustomScale,contactor,contacttel,payType,inRateType,rateType,rate,mrate,inRate," +
				"breedName,appAmount,payAmount,unAmount,rat," +
				"interest,mat,mgrAmount,mortName,mpVal," +
				"payDate,realDate,endDate,exteDate," +
				"managerName,comanager,referrals,loanId,ysRatMonth,ysRat,ysMatMonth,ysMat,riamount,trinterAmount,rmamount,trmgrAmount";
		...
		if(applyDt != null && applyDt.getRowCount() >0){
			for1:for(int i=0,count = applyDt.getRowCount();i<count;i++){
				...
				String payType = null;
				Integer rateType = null;
				Integer inRateType = null;
				Double rate = 0D;
				Double mrate = 0D;
				Double inRate = 0D;
				...
				BigDecimal riamount =  new BigDecimal(0).setScale(2);	
				BigDecimal trinterAmount =  new BigDecimal(0).setScale(2);
				BigDecimal rmamount =  new BigDecimal(0).setScale(2);	
				BigDecimal trmgrAmount =  new BigDecimal(0).setScale(2);
				...
				realDate = applyDt.getString(i, "realDate#yyyy-MM-dd");
				 ...
				 Long customerId = Long.parseLong(applyDt.getString(i, "customerId"));
				 //** 内部 利率    管理费率   还款方式 
				 payType = applyDt.getString(i,"payType");
				 rateType =Integer.parseInt(applyDt.getString(i,"rateType"));
				 String inrType = applyDt.getString(i,"inRateType");
				 if(StringHandler.isValidStr(inrType)){
					 inRateType =Integer.parseInt(inrType);
				 }
				 
				 rate = Double.parseDouble(applyDt.getString(i,"rate"));
				 mrate = Double.parseDouble(applyDt.getString(i,"mrate"));
				 inRate = Double.parseDouble(applyDt.getString(i,"inRate"));
						 ...
						 if(formId == id){
							 ...
							 if(!loanInvoceList.isEmpty() && loanInvoceList.size()>0){
								 for(LoanInvoceEntity loanInvoce :loanInvoceList){
									 ...
									 BigDecimal ysmat = loanInvoce.getYsMat();
									 if(StringHandler.isValidObj(ysmat)){
										 ysMat = ysmat.toString();
									 }
									BigDecimal ysrat = loanInvoce.getYsRat();
									if(StringHandler.isValidObj(ysrat)){
										 ysRat = ysrat.toString();
									}
									Integer ysmatRonth = loanInvoce.getYsRatMonth();
									if(StringHandler.isValidObj(ysmatRonth)){
										 ysRatMonth = ysmatRonth.toString();
									}
									Integer ysmatmonth = loanInvoce.getYsMatMonth();
									if(StringHandler.isValidObj(ysmatmonth)){
										ysMatMonth = ysmatmonth.toString();
									}
//									 realDate = DateUtil.dateFormatToStr(loanInvoce.getRealDate());
							 ...
							 List<CasualRecordsEntity> casualRecordsList = casualRecordsService.getEntityList(amountParams);
...
//									 rat = BigDecimalHandler.add2BigDecimal(rat, amountRecords.getRat());
							 ...
							 if(casualRecordsList.size()>0 && !casualRecordsList.isEmpty()){
								 for(CasualRecordsEntity x : casualRecordsList){
									 mat  = BigDecimalHandler.add2BigDecimal(mat, x.getFat(), 2);
								 }
							 }
							 ...
							 if(planList.size()>0 && !planList.isEmpty()){
								 for(PlanEntity plan : planList){
									 riamount = BigDecimalHandler.add2BigDecimal(riamount, plan.getRiamount());
									 trinterAmount = BigDecimalHandler.add2BigDecimal(trinterAmount, plan.getTrinterAmount());
									 rmamount = BigDecimalHandler.add2BigDecimal(rmamount, plan.getRmamount());
									 trmgrAmount = BigDecimalHandler.add2BigDecimal(trmgrAmount, plan.getTrmgrAmount());
									 rat = BigDecimalHandler.add2BigDecimal(rat, plan.getYinterest());
				 ...
				 Object[] datas = {
							...
							contacttel,payType,inRateType,rateType,rate,mrate,inRate,
							breedName,appAmount,payAmount,unAmount,
							rat,interest,mat,mgrAmount,mortName,
							mpVal,payDate,realDate,endDate,exteDate,
							managerName,comanager,referrals,loanId,
							ysRatMonth,ysRat,ysMatMonth,ysMat,riamount,trinterAmount,rmamount,trmgrAmount
					};
					dt.addRowData(datas);
			}
		}
	...
	public DataTable getDataSource(HashMap<String, Object> params) throws ServiceException {
		try {
			 DataTable dt = getResultList(new SHashMap<String, Object>(params), -1, -1);
			return dt;
		} catch (ServiceException e) {
			throw new ServiceException(e);
		}
