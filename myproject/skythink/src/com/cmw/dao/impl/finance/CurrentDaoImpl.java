	...
	public DataTable getCurrentList(SHashMap<String, Object> params, int start,int limit) throws DaoException {
		...
		try{
			colNames = "contractId,code,appAmount,breedName,payBank," +
					...
					"trinterAmount,trmgrAmount,trpenAmount,trdelAmount,"+
					"ytotalAmount,zprincipal,zinterest,zmgrAmount,zpenAmount," +
					"zdelAmount,ztotalAmount,realDate#yyyy-MM-dd,inRate,totMat,inRateType";
			
			sqlSb.append("select A.id as contractId,A.code,A.appAmount,E.name as breedName,A.payBank,")
				.append("A.payAccount,A.accName,H.ptotalPhases as totalPhases,isnull(B.paydPhases,0) as paydPhases,G.totalOverPharses,")
				...
				.append("H.yprincipal,H.yinterest,H.ymgrAmount,H.ypenAmount,H.ydelAmount,")
				.append("H.riamount,H.rmamount,H.rpamount,H.rdamount,")
				.append("H.trinterAmount,H.trmgrAmount,H.trpenAmount,H.trdelAmount,")
				.append("H.ytotalAmount,round(H.principal-H.yprincipal,2) as zprincipal,")
				.append("round(H.interest-H.yinterest-H.trinterAmount+H.riamount,2) as zinterest,")
				.append("round(H.mgrAmount-H.ymgrAmount-H.trmgrAmount+H.rmamount,2) as zmgrAmount,")
				.append("round(H.penAmount-H.ypenAmount-H.trpenAmount+H.rpamount,2) as zpenAmount,")
				.append("round(H.delAmount-H.ydelAmount-H.trdelAmount+H.rdamount,2) as zdelAmount,")
				.append("round(H.totalAmount-H.ytotalAmount-H.trinterAmount-H.trmgrAmount-H.trpenAmount-H.trdelAmount,2) as ztotalAmount, ")
				.append(" J.realDate,B.inRate ,xx.totMat,B.inRateType ")
				.append(" from fc_LoanContract A left join fc_Apply B ON A.formId=B.id ")
				.append(" left join (select sum(fat) as totMat,contractId from fc_CasualRecords where isenabled = '"+SysConstant.OPTION_ENABLED+"' group by contractId ) as xx on xx.contractId = A.id ")
				...
				.append(" left join (select contractId,count(1) as ptotalPhases,")
				...
				.append("round(sum(isnull(riamount,0)),2) as riamount,")
				.append("round(sum(isnull(rmamount,0)),2) as rmamount,")
				.append("round(sum(isnull(rpamount,0)),2) as rpamount,")
				.append("round(sum(isnull(rdamount,0)),2) as rdamount, ")
				...
				.append(" and A.id in (select contractId from fc_Plan where isenabled='"+SysConstant.OPTION_ENABLED+"' and status in ("+BussStateConstant.PLAN_STATUS_0+","+BussStateConstant.PLAN_STATUS_2+
