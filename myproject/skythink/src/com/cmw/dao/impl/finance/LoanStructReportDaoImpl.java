 ...
 */
@Description(remark="贷款业务结构报表DAO实现类",createDate="2013-08-09T00:00:00",author="程明卫")
@Repository("loanStructReportDao")
public class LoanStructReportDaoImpl extends GenericDaoAbs<LoanInvoceEntity, Long> implements LoanStructReportDaoInter {
	...
	private <K, V> DataTable getReportByMonth(SHashMap<K, V> map){
		...
		sbSql.append("select A.loanMain,A.inType,A.mainLoanType,A.loanType,A.breed,Datediff(MM,B.payDate,B.endDate) as loanLimit,")
		...
		.append(" where 1=1 ");
		String colNames = "loanMain,inType,mainLoanType,loanType,breed,loanLimit,appAmount,payAmounts,loanCount";
	...
	private <K, V> DataTable getReportByYear(SHashMap<K, V> map){
		...
		Integer state = map.getvalAsInt("state");
		sbSql.append("select A.loanMain,A.inType,A.mainLoanType,A.loanType,A.breed,Datediff(MM,B.payDate,B.endDate) as loanLimit,")
		.append("ROUND(B.appAmount/10000,2) as appAmount,ROUND((B.appAmount-ISNULL(D.yprincipals,0)),4) as zprincipals,ROUND(C.payAmounts,4) as payAmounts,C.loanCount ")
		.append(" from fc_Apply A inner join fc_LoanContract B on A.id = B.formId ")
		.append(" inner join (select count(1) as loanCount,sum(payAmount) as payAmounts,formId from fc_LoanInvoce ")
		.append(" where year(realDate)='").append(yearLoan).append("' and month(realDate)<='").append(monthLoan).append("' and state='").append(state).append("' ")
		.append(" group by formId) C on A.id = C.formId ")
		.append(" inner join (select isnull(sum(yprincipal),0) as yprincipals,contractId ")
		.append(" from fc_Plan group by contractId) D on B.id = D.contractId ")
		.append(" where 1=1 ");
		String colNames = "loanMain,inType,mainLoanType,loanType,breed,loanLimit,appAmount,zprincipals,payAmounts,loanCount";
		DataTable dt = findBySql(sbSql.toString(), colNames);
		return dt;
	}
}
