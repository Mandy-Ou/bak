...
public class LedgerDeatilDaoImpl  extends GenericDaoAbs<Object, Long> implements LedgerDeatilDaoInter{
	...
	public  <K, V> DataTable getResultList(SHashMap<K, V> map, int offset,
		...
		try{
			String colNames = "id,code,custType,customerId,loanMain,inType,loanType,breed,procId,inRate," +
					...
					"sourceType,referrals,payremark,sourceDesc,totalPhases,paydPhases,State,GuaId,LoanInvoceId,realDate#yyyy-MM-dd,inRateType";
			
			String sqlName = "A.id,A.code,A.custType,A.customerId,A.loanMain,A.inType,A.loanType,A.breed,A.procId,A.inRate," +
...
//					" (select max(lastDate) from fc_plan x where x.isenabled='"+SysConstant.OPTION_ENABLED+"' and B.contractId = x.contractId and A.State='"+BussStateConstant.FIN_APPLY_STATE_6+"' ) as realDate,A.inRateType" ;
					" A.currentDate as realDate,A.inRateType" ;
