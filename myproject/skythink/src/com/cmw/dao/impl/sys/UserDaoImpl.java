...
public class UserDaoImpl extends GenericDaoAbs<UserEntity, Long> implements UserDaoInter {
	...
	public <K, V> DataTable getResultList(SHashMap<K, V> params, int offset,
		...
		try{
			...
			sqlSb.append("select A.userId,A.isenabled,A.empName,A.sex,A.userName,")
				...
				.append(" A.email from ts_user A left join ts_user B on A.leader = B.userId ")
			...
			if(StringHandler.isValidObj(isLoan)){//
				sqlSb.append(" and C.isLoan = '"+isLoan+"' ");
			}
			String cmns =null;
			cmns = "userId,isenabled,empName,sex,userName,dataLevel,postName,leaderName,phone,tel,email";
	...
	public <K, V> DataTable getSerch(SHashMap<K, V> params, int offset,
		...
		try{
			...
			StringBuffer sqlSb = new StringBuffer();
			sqlSb.append("select A.userId,A.isenabled,A.empName,A.sex,A.userName,")
				.append(" A.dataLevel from ts_user A where A.isenabled !='"+SysConstant.OPTION_DEL+"' ");
			
			String serchId = params.getvalAsStr("serchId");
			if(StringHandler.isValidObj(serchId)){//
				sqlSb.append(" and A.empName like '"+serchId+"%'");
			}
			String cmns =null;
			cmns = "userId,isenabled,empName,sex,userName,dataLevel";
			long totalCount = getTotalCountBySql(sqlSb.toString());
			sqlSb.append(" order by A.userId desc ");
			DataTable dt = findBySqlPage(sqlSb.toString(),cmns, offset, pageSize,totalCount);
			return dt;
