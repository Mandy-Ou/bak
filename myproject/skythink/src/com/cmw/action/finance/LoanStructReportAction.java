...
//			map.put("state", BussStateConstant.LOANINVOCE_STATE_1);//取已放款数据
	...
	private String getHtmlDatas(DataTable dtMonthLoan, DataTable dtYearLoan) throws ServiceException{
		...
		/*--> step five : 生成HTML  <--*/
		String htmlData = makeHtml(isLoanInvoceAsOneCount, dtMonthLoan, dtYearLoan, dataList);
		return htmlData;
	}
	
	private String makeHtml(boolean isLoanInvoceAsOneCount, DataTable dtMonthLoan, DataTable dtYearLoan, List<JSONObject> dataList){
	...
	private Object[] getSumDataByDtYear(boolean isLoanInvoceAsOneCount,DataTable dtYearLoan) {
		int t_lcount = 0;
		double t_payAmount = 0d;
		double t_zprincipals = 0d;
		if(null != dtYearLoan && dtYearLoan.getRowCount() > 0){
			for(int i=0,count=dtYearLoan.getRowCount(); i<count; i++){
				Integer loanCount = dtYearLoan.getInteger(i,"loanCount");
				Double payAmounts = dtYearLoan.getDouble(i,"payAmounts");
				Double zprincipals = dtYearLoan.getDouble(i,"zprincipals");
				if(null == loanCount) loanCount= 0;
				if(null == payAmounts) payAmounts= 0d;
				if(null == zprincipals) zprincipals= 0d;
				if(!isLoanInvoceAsOneCount){
					t_lcount += loanCount;
				}else{
					t_lcount++;
				...
				t_payAmount += payAmounts;
				t_zprincipals += zprincipals;
			}
		}
		t_payAmount = StringHandler.Round(t_payAmount);
		t_zprincipals = StringHandler.Round(t_zprincipals);
		return new Object[]{t_lcount, t_payAmount, t_zprincipals};
	}
	/**
