...
public class LoanInvoceAction extends BaseAction {
	 ...
	 * 获取与预收dt
	 */
	public String getNeeYs()throws Exception {
		try {
			SHashMap<String, Object> map =  getQParams("id#L,name,custType#I,payName,regBank,account,eqopAmount,payAmount#o,startDate,endDate,ysRatMonth#I,ysRat#O,ysMatMonth#I,ysMat#O");
			map.put(SysConstant.USER_KEY, this.getCurUser());
			DataTable dt = loanInvoceService.getNeedYsList(map,getStart(),getLimit());
			result = (null == dt || dt.getRowCount() == 0) ? ResultMsg.NODATA : dt.getJsonArr();
		}catch (ServiceException ex){
			result = ResultMsg.getFailureMsg(this,ex.getMessage());
			if(null == result) result = ex.getMessage();
	 ...
	 * 获取与预收信息
	 */
	public String getYs()throws Exception {
		try {
			Long id = getLVal("id");
			if(id==null) new ServiceException(ServiceException.ID_IS_NULL);
	...
	public String saveYs()throws Exception {
		try {
			...
			Integer phases = getIVal("phases");
			String vtempCode = getVal("vtempCode");
			Integer sysId = getIVal("sysId");
			if(id==null) new ServiceException(ServiceException.ID_IS_NULL);
			SHashMap<String, Object> map =  new SHashMap<String, Object>();
			map.put(SysConstant.USER_KEY, this.getCurUser());
			map.put("id", id);
			...
			map.put("vtempCode", vtempCode);
			map.put("sysId", sysId);
			loanInvoceService.saveYs(map);
	...
	public String add()throws Exception {
		try {
			...
			Double  inRate = ApplyEntity.getInRate();
			String name = "";
			...
			String borBank = loanEntity.getBorBank();
			String borAccount = loanEntity.getBorAccount();
			map.put("regBank", borBank);
			map.put("account", borAccount);
...
//			map.put("account", payAccount);
	...
	public void getloaninvice(LoanInvoceEntity entity) throws ServiceException {
		...
		Double inRate = null;
		if(applyEntity != null){
			inRate = applyEntity.getInRate();
		}
		final Double inrate = inRate;
