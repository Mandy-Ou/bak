...
public class BalanceReportAction extends BaseAction {
	...
	private String getHtmlDatas(DataTable dtYearLoan) throws ServiceException{
		...
		/*--> step five : 生成HTML  <--*/
		String htmlData = makeHtml(isLoanInvoceAsOneCount, dtYearLoan, groupMap);
		return htmlData;
	}
	
	private String makeHtml(boolean isLoanInvoceAsOneCount, DataTable dtYearLoan, Map<String,List<JSONObject>> groupMap){
		if(null == groupMap || groupMap.size() == 0) return "";
		StringBuilder sbHtml = new StringBuilder();
		JSONObject sumData = getSumData(isLoanInvoceAsOneCount, dtYearLoan);
...
//		sbHtml.append(htmlTr);
	 ...
	 * @return
	 */
	private JSONObject getSumData(boolean isLoanInvoceAsOneCount, DataTable dtYearLoan){
		int t_ylcount = 0;
		double t_yzprincipals = 0d;
		double t_ypercent = 0d;
		Object[] dataArr = getSumDataByDt(isLoanInvoceAsOneCount,dtYearLoan);
		t_ylcount = (Integer)dataArr[0];
		t_yzprincipals = (Double)dataArr[1];
		
		/*--- 处理最后合计行  ---*/
		JSONObject sumData = new JSONObject();
		sumData.put("item", "合计");
		sumData.put("ylcount", t_ylcount);
		sumData.put("yzprincipals", t_yzprincipals);
		if(t_yzprincipals>0) t_ypercent = 100;
		sumData.put("ypercent", t_ypercent);
		return sumData;
	}
	
	private Object[] getSumDataByDt(boolean isLoanInvoceAsOneCount,DataTable dtYearLoan) {
		int t_lcount = 0;
		double t_amount = 0d;
		if(null != dtYearLoan && dtYearLoan.getRowCount() > 0){
			for(int i=0,count=dtYearLoan.getRowCount(); i<count; i++){
				...
				t_amount += zprincipals;
			}
		}
		t_amount = StringHandler.Round(t_amount);
		return new Object[]{t_lcount, t_amount};
	}
