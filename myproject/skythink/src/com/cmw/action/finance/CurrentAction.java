...
public class CurrentAction extends BaseAction{
	...
	public String get()throws Exception {
		try {
			...
			if(null != dt && dt.getRowCount() > 0){
				...
				String raVal = getRandomAlgorithmVal();
				if(StringHandler.isValidStr(raVal)) jsonObj.put(SysparamsConstant.RANDOM_ALGORITHM_KEY, raVal);
	...
	private String getRandomAlgorithmVal() throws ServiceException{
		SHashMap<String, Object> map = new SHashMap<String, Object>();
		map.put("isenabled", SysConstant.OPTION_ENABLED);
		map.put("recode", SqlUtil.LOGIC_EQ + SqlUtil.LOGIC + SysparamsConstant.RANDOM_ALGORITHM_KEY);
		SysparamsEntity sysparamsObj = sysparamsService.getEntity(map);
		String val = null;
		if(null != sysparamsObj) val = sysparamsObj.getVal();
		return val;
	}
	...
	/**
	 * 数据转换 -----〉 手动转换数据，随借随还 手动结清 数据状态
	 * @return
	 * @throws ServiceException
	 */
    public String settle() throws ServiceException {
    	try {
    		SHashMap<String, Object> params  = getQParams("bussType#I,contractId#L,rat,mat,pat,dat,rectDate,realDate");
