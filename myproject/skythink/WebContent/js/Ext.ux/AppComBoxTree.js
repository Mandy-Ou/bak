...
Ext.ux.tree.AppComboxTree = Ext.extend(Ext.form.TriggerField,{
	...
	setValue : function(vals){
		...
		if(vals || vals!=""){
			if(vals &&  Ext.isString(vals) && vals.indexOf(this.SIGIN) != -1){
			...
			if(val_txtArr && val_txtArr.length>1){
				...
				if(!vals || !Ext.isString(vals)) vals = "";
				this.selvals = vals;
				this.seltxts = vals;
			}	
		}else{
...
//			self.menu.hide();
	...
	comboxTree:function(comBox){
		...
		tree.on('dbclick',function(node,e){
			_this.addclickEvent(comBox,tree,node,e);
		});
		tree.addListener('click',function(node,e){
			_this.addclickEvent(comBox,tree,node,e);
		});
	},
	addclickEvent : function(comBox,tree,node,e){
		var _this = this;
		var selId = node.id;
			var Text = [];
			var joinId = [];
