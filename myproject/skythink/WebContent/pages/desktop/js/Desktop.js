...
Ext.Desktop = function(app){
   ...
   // var shortcuts = Ext.get('x-shortcuts');
 	var shortcuts = Ext.get('x-appsys-shortcuts');
 	var fixShortcuts = Ext.get('x-appfix-shortcuts');
    ...
    this.createWindow = function(config, calllback){
...
//        );
    	var baseCfg = {
         ...
         var win = null;
         if(config){
         	Ext.applyIf(config,baseCfg);
			win = new Ext.Window(config);
         }else{
         	win = calllback(baseCfg);
         }
...
//              Wrap to next row if we are not at the line start and this Window will go off the end
	...
	var shortcutChickHandler = function(e, t){
    ...
    //后台加载桌面图标绑定单击事件
    if(shortcuts){
        shortcuts.on('click', shortcutChickHandler);
    }
    //固定的系统绑定单击事件
    if(fixShortcuts){
        fixShortcuts.on('click', shortcutChickHandler);
    }
};
