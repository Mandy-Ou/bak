	...
	exports.WinEdit = {
		...
		getUrls : function(){
			...
			if(this.optionType == OPTION_TYPE.ADD){ //--> 新增
				...
				cfg.defaultVal = {countryd:subid};
				urls[URLCFG_KEYS.ADDURLCFG] = {url : './sysProvince_add.action',cfg : cfg};
			}else{
				/*--- 修改URL --*/
				var name = exports.WinEdit.parentCfg.self.apptree.getSelText();
				cfg = {params : {id:subid},defaultVal : {}};
				urls[URLCFG_KEYS.GETURLCFG] = {url : './sysProvince_get.action',cfg : cfg};
			}
			
			var id = this.appFrm.getValueByName("id");
			cfg = {params : {id:id}};
			/*--- 上一条 URL --*/
			urls[URLCFG_KEYS.PREURLCFG] = {url : './sysProvince_prev.action',cfg :cfg};
			/*--- 下一条 URL --*/
			urls[URLCFG_KEYS.NEXTURLCFG] = {url : './sysProvince_next.action',cfg :cfg};
			return urls;
		},
		/**
		 * 当数据保存成功后，执行刷新方法
		...
		createForm : function(){
			...
			/*国家ID*/
			var hid_countryd = FormUtil.getHidField({name:'countryd'});
			/*省份编号*/
			var txt_Code = FormUtil.getReadTxtField({fieldLabel : '编号',name:'code',allowBlank:false,width:200,maxLength:20});
			/*省份名称*/
			var txt_name = FormUtil.getTxtField({fieldLabel : '省份名称',name:'name',width:200,maxLength:25,allowBlank:false});
			/*英文名称*/
			...
			var layout_fields  = [hid_isenabled,hid_id,hid_countryd,txt_Code,txt_name,txt_Jname,txt_Twname,txt_Fname,txt_Koname,txa_remark];
			
			var frm_cfg = {title : '省份编辑',url:'./sysProvince_save.action'};
