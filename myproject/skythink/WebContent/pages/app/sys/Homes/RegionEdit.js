	...
	exports.WinEdit = {
		...
		getUrls : function(){
			...
			if(this.optionType == OPTION_TYPE.ADD){ //--> 新增
				/*--- 添加 URL --*/	
				cfg.defaultVal = {cityId:subid};
				urls[URLCFG_KEYS.ADDURLCFG] = {url : './sysRegion_add.action',cfg : cfg};
			}else{
				/*--- 修改URL --*/
				var name = exports.WinEdit.parentCfg.self.apptree.getSelText();
				cfg = {params : {id:subid},defaultVal : {}};
				urls[URLCFG_KEYS.GETURLCFG] = {url : './sysRegion_get.action',cfg : cfg};
			}
			
			var id = this.appFrm.getValueByName("id");
			cfg = {params : {id:id}};
			/*--- 上一条 URL --*/
			urls[URLCFG_KEYS.PREURLCFG] = {url : './sysRegion_prev.action',cfg :cfg};
			/*--- 下一条 URL --*/
			urls[URLCFG_KEYS.NEXTURLCFG] = {url : './sysRegion_next.action',cfg :cfg};
			return urls;
		},
		/**
		 * 当数据保存成功后，执行刷新方法
		...
		createForm : function(){
			...
			/*国家ID*/
			var hid_cityId = FormUtil.getHidField({name:'cityId'});
			/*城市编号*/
			var txt_Code = FormUtil.getReadTxtField({fieldLabel : '编号',name:'code',allowBlank:false,width:200,maxLength:20});
			/*城市名称*/
			var txt_name = FormUtil.getTxtField({fieldLabel : '地区名称',name:'name',width:200,maxLength:25,allowBlank:false});
			/*英文名称*/
			var txt_Jname = FormUtil.getTxtField({fieldLabel : '英文名称',name:'ename',width:200,maxLength:50});
			/*繁体中文名称*/
			var txt_Twname = FormUtil.getTxtField({fieldLabel : '繁体中文名称',name:'twname',width:200,maxLength:25});
			/*法文名称*/
			...
			var layout_fields  = [hid_isenabled,hid_id,hid_cityId,txt_Code,txt_name,txt_Jname,txt_Twname,txt_Fname,txt_Koname,txa_remark];
			
			var frm_cfg = {title : '地区编辑',url:'./sysRegion_save.action'};
