...
Ext.extend(skythink.cmw.sys.FormDiyMgr,Ext.util.MyObservable,{
	...
	getAppGrid : function(){
		...
		var wd = 90;
	  	var structure = [
			...
			{header : '允许输入最大长度',name : 'maxlength',width:90},
	...
	getAppPanel : function(){
		var self = this;
		var panel = new Ext.Panel({title:'自定义字段列表',height :(CLIENTHEIGHT - 137)});
			panel.add(this.globalMgr.getToolBar(self));
		var _panel = new Ext.Panel({height :(CLIENTHEIGHT - 137)/2 });
			this.globalMgr.appgrid = this.getAppGrid();
			_panel.add(this.globalMgr.appgrid);
		var _paneltwo = new Ext.Panel({title:'字段自定义属性列表',height :(CLIENTHEIGHT - 137)/2 });
		...
		getAppGrid2 : function(){
			...
			var wd = 100;
		  	var structure = [
				...
				{header : '是否必填',name : 'isRequired',width:wd,renderer : function(val){
					var xxx = typeof(val);
						switch (val) {
						case '0':
							val = "保留原设置";
