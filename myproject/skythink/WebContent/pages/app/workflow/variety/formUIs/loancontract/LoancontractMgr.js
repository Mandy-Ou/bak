...
Ext.extend(skythink.cmw.workflow.bussforms.LoancontractMgr,Ext.util.MyObservable,{
	...
	getToolBar : function(){
		...
		var barItems = [{
					...
					if(json_data.totalSize==0 || json_data.list == null){
							...
							if(arguments && arguments[0] != 'yes') return;
								...
								EventManager.get('./fcLoanContract_delete.action',{params:_params,sfn:function(jsondata){
									if(jsondata==-1){
										ExtUtil.alert({msg:"该借款合同已有放款单不能进行删除！"});
										return;
									}
									self.globalMgr.id = null;
									self.globalMgr.hide();
			...
			{
			text : Btn_Cfgs.LOAN_DATEIL_BTN_TXT,
			iconCls:Btn_Cfgs.PRINT_CLS,
			tooltip:Btn_Cfgs.LOAN_DATEIL_TIP_BTN_TXT,
			token : "打印借款合同",
			handler : function(){
				self.globalMgr.openPrintDialog({key:this.token,self:self});
			}
		...
		toolBar.hideButtons(Btn_Cfgs.LOAN_DOWNLOAD_BTN_TXT+","+Btn_Cfgs.LOAN_DATEIL_BTN_TXT+","+Btn_Cfgs.LOAN_SC_BTN_TXT);
		/**
		 * 获取button 配置信息
		 * @return {}
		 */
	...
	createDetailPnl : function(){
		...
		var htmlArrs_1 = [
						...
						'<tr>' +
							...
							'<th col="mrate">管理费率</th> <td col="mrate" colspan = 3>&nbsp;</td>' +
...
//		    nextUrl: '#NEXTURLCFG#',
	    ...
	    callback: {
	        sfn: function(jsonData) {
	        	...
	        	jsonData["appAmount"] =  Cmw.getThousandths(appAmount)+'元&nbsp;&nbsp;<span style="color:red;"><b>(大写：'+Cmw.cmycurd(appAmount)+')</b></span>';
		...
		openPrintDialog : function(parentCfg){
			...
			if(winkey == '生成借款合同'){
				...
				if(!_this.globalMgr.id){
					ExtUtil.alert({msg : '生成借款合同之前，必须先添加借款合同!'});
					return;				
				}
			}
