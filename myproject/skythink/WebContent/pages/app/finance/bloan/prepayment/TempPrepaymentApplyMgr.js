...
/**
 * 提前还款申请单 UI smartplatform_auto 2013-09-13 14:58:16
 */ 
cmw.skythink.TempPrepaymentApplyMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(cmw.skythink.TempPrepaymentApplyMgr,Ext.util.MyObservable,{
	initModule : function(tab,params){
		this.module = new Cmw.app.widget.AbsGContainerView({
			id : ComptIdMgr.tempPrepaymentApplyMgrId,
			tab : tab,
			params : params,
			hasTopSys : false,
	...
	getToolBar : function(){
		...
		var barItems = [{
			...
			handler : function(){
				EventManager.deleteData('./fcPrepayment_delete.action',{type:'grid',cmpt:_this.appgrid,optionType:OPTION_TYPE.DEL,self:_this});
			}
		}/*,{
			token : '导出',
		...
		submitApplyForm : function(_this){
			...
			var codeObj = _this.appgrid.getCmnVals("code");
			 ExtUtil.confirm({title:'提示',msg:'确定提交编号为："'+codeObj.code+'"的提前还款申请单?',fn:function(){
					...
					var params = {sysId:sysId,applyId:applyId,contractId:contractId,procId:null,bussProccCode:'B0002'};
