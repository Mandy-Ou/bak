...
//			var id = this.applyPanel.getValueByName("id");
			...
			var _this = this;
			this.applyPanel.reload({applyId : _this.applyId});
		},
		/**
		 * 加载客户信息数据
		 */
...
//					+ btnEditCoManHtml + btnDelCoManHtml + btnDetailCoManHtml;
		...
		createApplyPanel : function() {
			...
			var htmlArrs_1 = [
				'<tr>' +
					'<th ><span>*</span><label col="code">项目申请编号</label></th> <td col="code" >&nbsp;</td>' +
					'<th ><span>*</span><label col="loanMain">借款主体</label></th> <td col="loanMain" >&nbsp;</td>' +
					'<th ><span>*</span><label col="inType">行业分类</label></th> <td col="inType" >&nbsp;</td>' +
				'</tr>',
				'<tr>' +
					'<th ><span>*</span><label col="loanType">贷款方式</label></th> <td col="loanType" >&nbsp;</td>' +
					'<th ><span>*</span><label col="loanType">贷款方式</label></th> <td col="loanType" >&nbsp;</td>' +
					'<th ><span>*</span><label col="limitType">期限种类</label></th> <td col="limitType" >&nbsp;</td>' +
				'</tr>', 
				'<tr>' +
					'<th ><span>*</span><label col="appAmount">贷款金额</label></th> <td col="appAmount" >&nbsp;</td>' +
					'<th ><span>*</span><label col="loanLimit">贷款期限</label></th> <td col="loanLimit" >&nbsp;</td>' +
					'<th ><span>*</span><label col="payType">还款方式</label></th> <td col="payType" >&nbsp;</td>' +
				'</tr>',
				'<tr>' +
					'<th ><span>*</span><label col="breedName">业务品种</label></th> <td col="breedName" >&nbsp;</td>' +
					'<th ><span>*</span><label col="borBank">借款人银行</label></th> <td col="borBank" >&nbsp;</td>' +
					'<th ><span>*</span><label col="borAccount">借款人帐号</label></th> <td col="borAccount" >&nbsp;</td>' +
				'</tr>',
				
				'<tr>' +
					'<th col="sourceType">业务来源方式</th> <td col="sourceType" >&nbsp;</td>', 
					'<th col="appdate">申请日期</th> <td col="appdate" >&nbsp;</td>' +
					'<th col="referrals">业务介绍人</th> <td col="referrals" >&nbsp;</td>' +
				'</tr>' +
					'<th col="managerName">业务经理</th> <td col="managerName" >&nbsp;</td>' +
					'<th ><span>*</span><label col="rateType">利率类型</label></th> <td col="rateType" >&nbsp;</td>' +
					'<th ><span>*</span><label col="isadvance">是否提前收息</label></th> <td col="isadvance" >&nbsp;</td>' +
				'</tr>', 
				'<tr>' +
					'<th col="mgrtype">管理费收取方式</th> <td col="mgrtype" >&nbsp;</td>' +
					'<th col="mrate">管理费率</th> <td col="mrate" >&nbsp;</td>' +
					'<th ><label col="inRateType">内部利率类型</label></th> <td col="inRateType" >&nbsp;</td>' +
				'</tr>', 
				'<tr>' +
...
//					'<th col="arate">提前还款费率</th> <td col="arate" >&nbsp;</td>' +
				...
				FORMDIY_DETAIL_KEY,
				'<tr>' +
					'<th col="GuaId">担保人</th> <td col="GuaId"  colspan=5 >&nbsp;</td>' +
				'</tr>',
				'<tr height="50">' +
					'<th col="reason">申请原因</th> <td col="reason" colspan=5>&nbsp;</td>' +
				'</tr>',
				'<tr  height="50">' +
					'<th col="payremark">贷款用途</th> <td col="payremark"  colspan=5 >&nbsp;</td>' +
				'</tr>',
				'<tr  height="50">' +
					'<th col="sourceDesc">还款计划及资金来源</th> <td col="sourceDesc"  colspan=5 >&nbsp;</td>' +
				'</tr>'
				];
			var detailCfgs_1 = [{
				title : '借款申请单信息',
			    ...
			    callback: {
			        sfn: function(jsonData) {
			        	...
			        	jsonData["rateType"] = Render_dataSource.rateTypeRender(jsonData["rateType"])+'('+jsonData["rate"]+'%)';
			        	jsonData["isadvance"] = Render_dataSource.isDefaultRenders(jsonData["isadvance"]);
			        	jsonData["loanMain"] = Render_dataSource.gvlistRender('200000',jsonData["loanMain"]);
			        	jsonData["inType"] = Render_dataSource.gvlistRender('200001',jsonData["inType"]);
			        	var loanType = jsonData["loanType"];
			        	if(loanType.indexOf(",")!=-1){
			        		var type = loanType.split(",");
			        		var lType = "";
			        		for (var index = 0; index < type.length; index++) {
			        			lType += Render_dataSource.gvlistRender('200002',type[index])+',';
			        		}
			        		lType = lType.substring(0,lType.length-1);
			        		jsonData["loanType"] = lType;
			        	}else{
			        		jsonData["loanType"] = Render_dataSource.gvlistRender('200002',jsonData["loanType"]);
			        	}
			        	...
			        	jsonData["limitType"] = Render_dataSource.gvlistRender('200003',jsonData["limitType"]);
			        	jsonData["sourceType"] = jsonData["sourceType"] ? Render_dataSource.gvlistRender('200004',jsonData["sourceType"]):'';
			        	jsonData["appAmount"] =   Cmw.getThousandths(jsonData["appAmount"])+'元&nbsp;&nbsp;<span style="color:red;">(大写：'+Cmw.cmycurd(jsonData["appAmount"])+')</span>';
			        	jsonData["mgrtype"] =Render_dataSource.mgrtypeRender(jsonData["mgrtype"].toString());
			        	...
			        	jsonData["inRate"] =jsonData["inRate"]+'%';// 公司内部利率
			        	var inRateType = jsonData["inRateType"];
			        	if(inRateType){
			        		jsonData["inRateType"] = Render_dataSource.rateTypeRender(jsonData["inRateType"]);	
			        	}else{
			        		jsonData["inRateType"] = "";
			        	}
			        }
