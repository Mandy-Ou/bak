...
/**
 * 个企业客户暂存申请单 UI smartplatform_auto 2012-12-29 09:48:16
 */ 
cmw.skythink.TempEntCustApplyMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(cmw.skythink.TempEntCustApplyMgr,Ext.util.MyObservable,{
	initModule : function(tab,params){
		this.module = new Cmw.app.widget.AbsGContainerView({
			id : ComptIdMgr.tempEntCustApplyMgrId,
			tab : tab,
			params : params,
			hasTopSys : false,
	...
	getQueryFrm : function(){
		...
		var txt_tradNumber = FormUtil.getTxtField({fieldLabel : '营业执照号',name:'tradNumber',width : 150});
	...
	getToolBar : function(){
		...
		var barItems = [{
			...
			handler : function(){
				self.globalMgr.detailApplyForm(self);
			}
		},{type:"sp"},{/*提交*/
			token : '提交',
			...
			handler : function(){
				EventManager.deleteData('./fcApply_delete.action',{type:'grid',cmpt:self.appgrid,optionType:OPTION_TYPE.DEL,self:self});
			}
		},{
			token : '导出',
	...
	getAppGrid : function(){
		var _this = this;
		    ...
		    {header: '公司内部利率',  name: 'inRate' ,width: 80,
			 	renderer : function(val,m){
		    		m.css='x-grid-back-red'; 
		    		if(val || val==0) val += '%';
		    		return val ;
	...
	globalMgr : {
		getQparams : function(_this){
			if(!_this.queryFrm || !_this.queryFrm["getValues"]) return;
		...
		doApplyByOp : function(_this,op){
			var params = {applyId:null,customerId:null,custType:null};
			if(op == OPTION_TYPE.EDIT){//编辑 时传入申请单ID，客户ID，客户类型
				var applyId = _this.appgrid.getSelId();
				if(!applyId) return;
				var data = _this.appgrid.getCmnVals(["customerId","custType"]);
