...
//			this.fieldChangeListener(null);
			...
			if(inRateType){
				inRateType = Render_dataSource.rateTypeRender(jsonData["inRateType"])
			}else{
				inRateType = "";
			}
		...
		saveData : function() {
			...
			if(oddAmount>0){
				 Ext.MessageBox.show({title:'提示',msg:'本次收款实收合计：<span style="color:blue;font-weight:bold;">'+tat+'</span>元，' +
				 		'应找回金额:<span style="color:blue;font-weight:bold;">'+oddAmount+'</span>元，是否将找回金额存入预收款帐户?',
				 		icon : Ext.MessageBox.QUESTION,buttons: Ext.MessageBox.YESNOCANCEL,fn:function(btn){
				 	if(btn == 'yes'){
				 		save(true);
				 	}else if(btn == 'no'){
				 		save();
