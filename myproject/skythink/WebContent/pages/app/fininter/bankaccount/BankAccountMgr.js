...
/**
 * 银行账号配置管理 UI smartplatform_auto 2013-04-20 15:38:16
 */ 
skythink.cmw.fininter.BankAccountMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(skythink.cmw.fininter.BankAccountMgr,Ext.util.MyObservable,{
	...
	getQueryFrm : function(){
		...
		var txt_name = FormUtil.getTxtField({fieldLabel : '账户名称',name:'name'});
		var txt_bankName = FormUtil.getTxtField({fieldLabel : '开户行',name:'bankName'});
		 var layout_fields = [{cmns:FormUtil.CMN_THREE,fields:[txt_code,txt_name,txt_bankName]}]
	...
	getAppGrid : function(){
	    ...
	    var structure = [{header : '可用标识',name : 'isenabled',width:60,renderer : Render_dataSource.isenabledRender},
						{header:'编号' ,name:'code' ,width:100},
						{header:'账户名称' ,name:'name' ,width:100},
						{header:'银行账号' ,name:'actNumber' ,width:120},
						{header:'开户行' ,name:'bankName' ,width:160},
						{header:'银行账号ID' ,name:'refId' ,width:100},
		...
		synchronousData : function(_this){
			 ...
			 fn:function(btn){
			 	if(btn != 'yes') return;
					...
					EventManager.get('./fsBankAccount_synchronous.action',{params:{finsysId:_this.globalMgr.finsyscfgId},
