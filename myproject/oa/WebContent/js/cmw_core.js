...
//		var self = this;
	...
	/*---------------------- OA ADD CODE START --------------------------------*/
	/**
	 * 根据 txr_cmw_oaApp 创建或激活指定 tabId 的 tab 对象
	 * 以JSON对象形式返回
	 * @param {} tabId tab面板对象ID
	 * @param {} params 参数对象
	 * @param String url   js 界面文件地址
	 * @param String tabTitle TabPanel 的标题
	 */
	activeTabByOa:function(tabId,params,url,tabTitle,uiActiveType){
		if(!txr_cmw_oaApp){
			ExtUtil.error({msg : '"txr_cmw_oaApp" 对象为空，无法激活  "'+tabId+'" 对象!'});
			return;
		}
		if(url && !tabTitle){
			ExtUtil.error({msg : '如果加载自定义 url 的JS文件，必须为 Tab 面板指定 title 值!'});
		...
		this.putUiActiveTypePars(params, uiActiveType);
		txr_cmw_oaApp.activeByCustTabId(false,tabId,params,url,tabTitle);
	},
	/**
	 * 根据 txr_cmw_oaApp 创建或激活 流程审批面板
	 * 以JSON对象形式返回
	 * @param {} tabId tab面板对象ID
	 * @param {} params 参数对象
	 * @param String url   js 界面文件地址
	 * @param String tabTitle TabPanel 的标题
	 */
	activeAuditFlowTab:function(params,tabTitle,uiActiveType){
		if(!txr_cmw_oaApp){
			ExtUtil.error({msg : '"txr_cmw_oaApp" 对象为空，无法激活  "'+tabId+'" 对象!'});
			return;
		}
	...
	/**
	 * 根据 menuId 通过 txr_cmw_oaApp 创建或激活指定的 tab 对象
	 * 以JSON对象形式返回
	 * @param String menuId   menuId 菜单ID
	 * @param {} params 参数对象
	 */
	activeTabByMenuId:function(menuId,params,uiActiveType){
		if(!txr_cmw_oaApp){
			ExtUtil.error({msg : '"txr_cmw_oaApp" 对象为空，无法激活  "'+tabId+'" 对象!'});
			return;
		}
		this.putUiActiveTypePars(params, uiActiveType);
		txr_cmw_oaApp.activeByCustTabId(true,menuId,params);
	},
	/**
	 * 将激活参数放入参数中
	 * @param {} params	要传递的参数
	 * @param {} uiActiveType	UI激活方式
	 */
	putUiActiveTypePars : function(params,uiActiveType){
		if(!uiActiveType) uiActiveType = OA_UIActiveType.tipInstance;/*当 UI 存在时，给出界面已存在的提示*/
		params.uiActiveType = uiActiveType;
	},
	/**
	 * 根据 txr_cmw_oaApp 隐藏指定的 tab 对象
	 * 以JSON对象形式返回
	 * @param {} AppTabTreeWin 对象
	 * @param {} tab tab面板对象
	 */
	hideTabByOa:function(tab){
		if(!txr_cmw_oaApp){
			ExtUtil.error({msg : '"txr_cmw_oaApp" 对象为空，无法隐藏  "'+tab.id+'" 对象!'});
			return;
		}
		txr_cmw_oaApp.hideTabItem(tab);
	},
	/**
	 * 根据 txr_cmw_oaApp 显示指定的 tab 对象
	 * 以JSON对象形式返回
	 * @param {} tab tab面板对象
	 */
	showTabByOa:function(tab,tabTitle){
		if(!txr_cmw_oaApp){
			ExtUtil.error({msg : '"txr_cmw_oaApp" 对象为空，无法显示 "'+tab.id+'" 对象!'});
			return;
		}
		txr_cmw_oaApp.showTabItem(tab);
	}/*---------------------- OA ADD CODE END --------------------------------*/
}
