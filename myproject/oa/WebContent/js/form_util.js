...
var FormUtil = {
	...
	/**
	 * 特殊说明：
	 *        这个是在系统原有的基础上添加的额外的时间控件，
	 *        包含的文件有 ：
	 *        1.oa/WebContent/js/Ext.ux下的DateTimeField.js、Spinner.js、SpinnerField.js
	 * 		  2./oa/WebContent/css下的Spinner.css
	 * 		  3.如果不需要，除了删除以上几个文件，还需要在/oa/WebContent/js下的cmw_appDefine.js中将加载的DateTimeField.js、Spinner.js、SpinnerField.js去除。
	 * 
	 * 根据配置参数获取一个 DateField 对象
	 * @param {} cfg	DateField 配置参数   时间精确到时分秒。也可以通过 配置项 format :'Y-m-d H:i:s' 进行格式化配置。
	 * @return {}	返回一个 DateField 对象
	 */
	getDateFormatField : function(cfg){
		if(!cfg.width) cfg.width = this.WIDTH;
		if(cfg.hasOwnProperty("allowBlank") && !cfg.allowBlank) cfg.fieldLabel = this.REQUIREDHTML + cfg.fieldLabel;
		if(!cfg.format)cfg.format =  'Y-m-d H:i:s';
		if(cfg.isdeaulft){
		 cfg.value = CURENT_DATE();
		}
		if(cfg.format == 'Y' || cfg.format == 'm' || cfg.format == 'Y-m'){
		...
		return new Ext.ux.form.DateTimeField(cfg);
		//return new Ext.form.DateField(cfg);
	},
	/**
	...
	getLCboField : function(cfg){
		...
		cfg = Ext.applyIf(
			cfg,{store : _store,emptyText:'请选择',mode : 'local',triggerAction : 'all',hiddenName:cfg.name,
...
//			border : false,buttonAlign : 'center'},cfg);
		...
		if(isThreeCmn){	//存在三行布局的情况
			for(var i=0,count=fldGroups.length; i<count; i++){
				...
				if(cmns == this.CMN_THREE){
					...
					layoutFlds = this.getTwoLayout(fields,{first:.33,second:.66});
				}else if(cmns == this.CMN_TWO_LEFT){
					layoutFlds = this.getTwoLayout(fields,{first:.66,second:.33});
				}
	 ...
	 */
	setFieldValues : function(fields,json_data){
