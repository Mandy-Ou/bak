 ...
 */ 
skythink.cmw.fininter.SettleMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(skythink.cmw.fininter.SettleMgr,Ext.util.MyObservable,{
	...
	getQueryFrm : function(){
		var self =this;
		var txt_code= FormUtil.getTxtField({fieldLabel : '结算方式编号',name:'code'});
		var txt_name = FormUtil.getTxtField({fieldLabel : '结算方式名称',name:'name'});
	...
	getAppGrid : function(){
	    ...
	    var structure = [{header : '可用标识',name : 'isenabled',width:60,renderer : Render_dataSource.isenabledRender},
						{header:'结算方式编号' ,name:'code' ,width:100},
						{header:'结算方式名称' ,name:'name' ,width:100},
						{header:'结算方式ID' ,name:'refId' ,width:100},
		...
		synchronousData : function(_this){
			 ...
			 fn:function(btn){
			 	if(btn != 'yes') return;
					...
					EventManager.get('./fsSettle_synchronous.action',{params:{finsysId:_this.globalMgr.finsyscfgId},
