...
Ext.extend(skythink.cmw.fininter.FinSysCfgMgr,Ext.util.MyObservable,{
	...
	getToolBar : function(){
		...
		var barItems = [{
			...
			handler : function(){
				...
				EventManager.get('./fsFinSysCfg_isNotLink.action',{prams:{id:nodeId},ffn:function(){
					ExtUtil.info({msg:Fs_Msg_SysTip.msg_isnotlink});
					return;
				}});
				EventManager.disabledData('./fsFinSysCfg_delete.action',{type:'tree',cmpt:self.apptree,optionType:OPTION_TYPE.DISABLED,self:self});
			}
		}];
	...
	getAppPanel : function(){
		var _this = this;
		var appPanel = new Ext.Panel({border:false,autoWidth:true});
