...
/**
 * @description 待审的借款申请单管理 UI 
 * @date 2014-02-07 17:39
 */ 
cmw.oa.AuditLoanApplyMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(cmw.oa.AuditLoanApplyMgr,Ext.util.MyObservable,{
	queryFields : null,
	...
	getToolBar : function(){
		...
		var barItems = [{token:tokenMgr.QUERY_TXT,text:Btn_Cfgs.QUERY_BTN_TXT,iconCls:Btn_Cfgs.QUERY_CLS,handler:function(){
					...
					{token:tokenMgr.DELETE_TXT,text:Btn_Cfgs.DELETE_BTN_TXT,iconCls:Btn_Cfgs.DELETE_CLS,handler:function(){
						EventManager.deleteData('./oaLoanApply_delete.action',{type:'grid',cmpt:_this.appgrid,optionType:OPTION_TYPE.DEL,self:_this});
					}}
					];
	...
	getQueryFrm : function(){
		...
		var cmpt_loanDate = FormUtil.getMyCompositeField({fieldLabel:'借款日期',name:'cmpt_loanDate',width:width,
		 ...
		 var layout_fields = [{cmns:FormUtil.CMN_THREE,fields:[txt_code,txt_loanMan,rad_rendType,cmpt_amount,cmpt_loanDate]}]
	...
	getAppGrid : function(){
		...
		var appgrid_1 = new Ext.ux.grid.AppGrid({
		    ...
		    url: './oaLoanApply_auditlist.action',
		  //  needPage: true,
		    isLoad : false,
		    keyField: 'id',
		    listeners:{
