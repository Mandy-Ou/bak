...
Ext.extend(skythink.cmw.workflow.bussforms.MortgageMgr,Ext.util.MyObservable,{
	...
	notify : function(data){
		...
		this.globalMgr.appgrid.reload({gtype:this.globalMgr.gvlistid,formId:this.globalMgr.formId});
	},
	getToolBar : function(){
	...
	getMgDetail : function(){
		...
		var htmlArrs = [
			'<tr>',
			    '<th><span>*</span><label col="name">抵押物名称：</label></th>',
			    	'<td col="name">&nbsp;</td>',
			    '<th><span>*</span><label col="quantity">数量：</label></th>',
			    	'<td  col="quantity">&nbsp;</td>',
			   	'<th><span>*</span><label col="mman">抵押人：</label></th>',
			   		'<td col="mman">&nbsp;</td>',
			'</tr>',
			'<tr>',
			 	'<th ><span>*</span><label col="oldVal" >原价值（元）：</label></th>',
			    	'<td  col="oldVal">&nbsp;</td>',
			    '<th><span>*</span><label col="mapVal" >抵押价值：</label></th>',
	...
	getAppPanel : function(){
		var panel = new Ext.Panel({border:false,width:900,height : 660,autoScroll:true});
	...
	refresh:function(optionType,data){
		...
		if(optionType==OPTION_TYPE.DEL){
			...
			var _this = this;
		this.globalMgr.appgrid.reload({gtype:this.globalMgr.gvlistid,formId:formId},function(){
			switch(optionType){
	...
	createSysPanel : function(self){
		var sysPanel = new Ext.Panel({region:'west',title:'抵押物列表',border:false,autoScroll:true,width:125});
	...
	getToolBar : function(self){
		var barItems = [{
			...
			handler : function(){
				EventManager.deleteData('./fcMortgage_delete.action',{type:'grid',cmpt:self.globalMgr.appgrid,optionType:OPTION_TYPE.DEL,self:self});
			}
		}];
		...
		for(var i=0,count=buttons.length; i<count; i++){
			...
			if(btnCfg.text==Btn_Cfgs.DELETE_BTN_TXT){
...
//					if(winkey=="添加" || winkey=="编辑"){
						winModule="MortgageEdit";
//					}
					Cmw.importPackage('pages/app/workflow/variety/formUIs/mort/'+winModule,function(module) {
