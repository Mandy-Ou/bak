	...
	exports.WinEdit = {
		...
		createForm : function(){
			var _this = this;
				...
				var setdayTypeChangeListener = function(combox,newValue,oldValue){
					...
					if(newValue == "1"){
						...
						EventManager.get('./sysSysparams_getParamsName.action',{params:{name:'PAYDAY_SET',sysid : _this.sysid},
			...
			var dob_frate = FormUtil.getDoubleField({
				...
				var layout_fields = [
					txt_id,hid_formId,hid_custType,hid_customerId,
					...
					{cmns: FormUtil.CMN_THREE,fields: [compt_payDay,bdat_payDate, int_endDate,rad_isadvance,cbo_mgrtype,dob_mrate,dob_prate ,dob_arate,dob_urate,dob_frate,bdat_doDate]},
					txt_clause
				];
		...
		saveData : function(){
			...
			if(setdayType==3){
				if(!payDay || payDay==""|| payDay==null ){
					ExtUtil.alert({msg:"结算日期为其他结算日期的时候，每期结算日不能为空!"});
					return ;
				}
			}
