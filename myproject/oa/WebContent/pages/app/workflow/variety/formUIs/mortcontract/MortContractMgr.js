...
Ext.extend(skythink.cmw.workflow.bussforms.LoancontractMgr,Ext.util.MyObservable,{
	initModule : function(tab,params){
		...
		this.module = new Cmw.app.widget.AbsPanelView({
			...
			createAttachMentFs : this.createAttachMentFs,
	...
	getAppCmpt : function(){
		...
		appPanel.add({items:[this.getToolBar(),this.getAppGrid(),this.getMgDetail()]});
		return appPanel;
	},
	/**
	 * 刷新方法
	...
	getToolBar : function(){
		var self = this;
		var barItems = [{
			text : Btn_Cfgs.MORTCONTRACT_ADD_TXT,
			iconCls:'page_add',
			tooltip:Btn_Cfgs.MORTCONTRACT_ADD_TIP_BTN_TXT,
			handler : function(){
				EventManager.get('./fcLoanContract_get.action',{params:{ formId :  self.globalMgr.formId},sfn :function(data){
					if(data==-1){
						ExtUtil.alert({msg:"没有添加借款合同不能进行添加抵押合同！"});
						return;
					}
					self.globalMgr.winEdit.show({key:"添加抵押合同",self:self});
				}});
				
			}
		},{type:"sp"},{
			text : Btn_Cfgs.MORTCONTRACT_EDIT_BTN_TXT,
			iconCls:'page_edit',
			tooltip:Btn_Cfgs.MORTCONTRACT_EDIE_TIP_BTN_TXT,
			handler : function(){
				var selId = self.globalMgr.appgrid.getSelId();
				if(selId!=null){
//					self.globalMgr.EditBtn.enable();	
					self.globalMgr.winEdit.show({key:"编辑抵押合同",optionType:OPTION_TYPE.EDIT,self:self});
				}
				
			}
		},{type:"sp"},{
			text : Btn_Cfgs.MORTCONTRACT_DEL_BTN_TXT,
			iconCls:'page_delete',
			tooltip:Btn_Cfgs.MORTCONTRACT_DEL_TIP_BTN_TXT,
			handler : function(){
				EventManager.deleteData('./fcMortContract_delete.action',{type:'grid',cmpt:self.globalMgr.appgrid,optionType:OPTION_TYPE.DEL,self:self});
			}
		},{
			text :Btn_Cfgs.MORTCONTRACT_SC_BTN_TXT,
			iconCls:'page_query',
			tooltip:Btn_Cfgs.MORTCONTRACT_SC_TIP_BTN_TXT,
			handler : function(){
				self.globalMgr.query(self);
			}
		},
			{
			text : Btn_Cfgs.MORTCONTRACT_DATEIL_BTN_TXT,
			iconCls:'page_add',
			tooltip:Btn_Cfgs.MORTCONTRACT_DATEIL_TIP_BTN_TXT,
			handler : function(){
				self.globalMgr.winEdit.show({key:"打印抵押合同",self:self});
			}
		},{
			text : Btn_Cfgs.MORTCONTRACT_DOWNLOAD_BTN_TXT,
			iconCls:'page_edit',
			tooltip:Btn_Cfgs.MORTCONTRACT_DOWNLOAD_TIP_BTN_TXT,
			handler : function(){
				self.globalMgr.winEdit.show({key:"抵押合同模板下载",optionType:OPTION_TYPE.EDIT,self:self});
			}
		}];
		
		toolBar = new Ext.ux.toolbar.MyCmwToolbar({aligin:'left',controls:barItems});
		toolBar.hideButtons(Btn_Cfgs.MORTCONTRACT_SC_BTN_TXT+','+Btn_Cfgs.MORTCONTRACT_DATEIL_BTN_TXT+','+Btn_Cfgs.MORTCONTRACT_DOWNLOAD_BTN_TXT);
...
//		self.globalMgr.getBtn(buttons,self);
	...
	getMgDetail : function(){
			...
			var htmlArrs = [
				'<tr>',
				    '<th><span>*</span><label col="code">抵押合同编号：</label></th>',
				    '<td  col="code">&nbsp;</td>',
				...
				'<tr>',
				    ...
				    '<td  col="conAmount" >&nbsp;</td>',
				    '<th><label col="mortName">抵押物：</laebl></th>',
				    '<td  col="mortName"  colspan="3">&nbsp;</td>',
				'</tr>',
//				FORMDIY_DETAIL_KEY,
				 '<tr>',
...
//				 formDiyCfg : {sysId:sysId,formdiyCode:gvlistId,formIdName:'id'},
				 callback:{
				 	sfn:function(jsonData){
			        	...
			        	var params = {formId:formId,formType:ATTACHMENT_FORMTYPE.FType_9,sysId:sysId,isNotDisenbaled:true};
			        	_this.attachMentFs.reload(params);
				 	}
				 }
			}];
			var cfg = {autoWidth:true,detailCfgs:detailCfgs,hidden : true,border:false};
	...
	globalMgr : {
		...
		createAttachMentFs : function(_this){
			 ...
			 *  ---- */
			var attachMentFs = new  Ext.ux.AppAttachmentFs({title:'抵押合同附件列表',isLoad:false,dir : 'mortcontract_path',isSave:true,border:false,isNotDisenbaled:true});
			return attachMentFs;
		},
		/**
		 * 删除合同
		 */
		 btuclick :function(self){
		 	if(arguments && arguments[0] == 'yes'){
				EventManager.get('./fcMortContract_delete.action',{params:{ formId : self.globalMgr.formId}},{optionType:OPTION_TYPE.DEL,self:self});
				self.globalMgr.hide();
...
//				self.globalMgr.DelBtn.disable();
		  ...
		  */
		  getBtn : function(buttons,self){
			...
			if(btnCfg.text==Btn_Cfgs.MORTCONTRACT_EDIT_BTN_TXT){
				self.globalMgr.EditBtn=btnCfg;
				self.globalMgr.EditBtn.disable();
			}
			if(btnCfg.text== Btn_Cfgs.MORTCONTRACT_DEL_BTN_TXT){
		...
		detailPanel_1 : null,
			...
			show : function(parentCfg){
				...
				parent={
					...
					appgrid:_this.globalMgr.appgrid,
					EditBtn:_this.globalMgr.EditBtn,
				...
				if(_this.appCmpts[winkey]){
					...
					var winModule=null;
					if(winkey=="添加抵押合同" || winkey=="编辑抵押合同"){
						winModule="MortContractEdit";
					}
					Cmw.importPackage('pages/app/workflow/variety/formUIs/mortcontract/'+winModule,function(module) {
