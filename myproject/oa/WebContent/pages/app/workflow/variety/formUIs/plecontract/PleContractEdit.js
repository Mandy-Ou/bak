...
define(function(require, exports) {
	exports.WinEdit = {
		...
		createAppWindow : function(){
			...
			this.appWin = new Ext.ux.window.AbsEditWindow({title:'质押合同信息编辑'+'<span style="color:red">>>>填写的质押合同编号必须唯一</span>',width:660,getUrls:this.getUrls,appFrm :this.appFrm,autoScroll : true,
		...
		getUrls : function(){
			...
			if(this.optionType == OPTION_TYPE.ADD){ //--> 新增
				...
				urls[URLCFG_KEYS.ADDURLCFG] = {url : './fcPleContract_add.action',cfg : cfg};
			}else{
				/*--- 修改URL --*/
				...
				urls[URLCFG_KEYS.GETURLCFG] = {url : './fcPleContract_get.action',cfg : cfg};
			}
			var id = this.appFrm.getValueByName("id");
			var id = this.appFrm.getValueByName("formId");     
			cfg = {params : {id:id,sfn:function(){}}};
			/*--- 上一条 URL --*/
			urls[URLCFG_KEYS.PREURLCFG] = {url : './fcPleContract_prev.action',cfg :cfg};
			/*--- 下一条 URL --*/
			urls[URLCFG_KEYS.NEXTURLCFG] = {url : './fcPleContract_next.action',cfg :cfg};
		...
		createForm : function(){
			...
			var layout_fields = [hid_custType,hid_customerId,hid_id,hid_formId,hid_loanConId,hid_pleIds,{
			    ...
			    {cmns: FormUtil.CMN_TWO,fields: [dob_rate,doub_conAmount, txt_assMan, bdat_startDate, 
...
//			    title: '质押合同信息编辑',
		...
		getToolBar : function(){
			...
			var barItems = [{
				...
				if(_this.PleContractDialog){
					_this.PleContractDialog.show(parentCfg);
				}else{
						Cmw.importPackage('/pages/app/dialogbox/PleContractDialogbox',function(module) {
						 	_this.PleContractDialog = module.DialogBox;
						 	_this.PleContractDialog.show(parentCfg);
				  		});
					}
				}
