...
Ext.extend(skythink.cmw.workflow.bussforms.custAttMgr,Ext.util.MyObservable,{
	...
	getAppCmpt  : function(){
		var that = this ; 
		this.appPanel = new Ext.Panel({renderTo: Ext.getBody(),bodyStyle  : 'padding: 0 8 20 0;',autoHeight:true,autoWidth : true,border : false});
	...
	getToolBat : function(_this){
		...
		htmlArrs : function(_this){
			...
			EventManager.get('./sysMatTemp_getMat.action',{params:{breed : _this.globalMgr.breed,custType : _this.params.custType},sfn:function(jsonData){
				if(jsonData){
					if(!jsonData.id){
						_this.globalMgr.toolBar.disable();
						_this.appPanel.updata("该贷款业务品种下暂无数据！",false);
						return;
					}
					...
					Cmw.importPackage('pages/app/sys/mat/custAttachment',function(module) {
					 	var htmlPnl =  module.moduleUI;
					 	_this.globalMgr.htmlPnl = htmlPnl.getModule(_this);
					 	...
					 	htmlPnl.callback = function(data){
