...
define(function(require, exports) {
	exports.moduleUI = {
		...
		loadData : function() {
			var _this = this;
			EventManager.get('./fcApply_detail.action',{params:{applyId:_this.applyId},sfn:function(json_data){
				_this.json_data = json_data;
				var custType = json_data.custType;
				_this.custType = custType;
				_this.customerId = json_data.customerId;
				var activePanel = _this.showCustPanle(custType,json_data);
				activePanel.reload({json_data : json_data},true);
		...
		getOneCustDetailPanel : function() {
			var _this = this;
			var htmlArrs_1 = [
					'<tr><th col="code">申请单编号</th> <td col="code" >&nbsp;</td><th col="name">客户简要资料</th> <td col="name" >&nbsp;</td><th col="breedName">业务品种</th> <td col="breedName" >&nbsp;</td></tr>',
					...
					'<tr><th col="payType">还款方式</th> <td col="payType" >&nbsp;</td><th col="managerName">业务经理</th> <td col="managerName" >&nbsp;</td><th col="appdate">申请日期</th> <td col="appdate" >&nbsp;</td></tr>'];
			var detailCfgs_1 = [{
						cmns : 'THREE',
						/* ONE , TWO , THREE */
						model : 'single',
						...
						callback : {
							sfn : function(jsonData) {
								...
								var applyAid = _this.addAHtml(jsonData,"code","单击查看贷款申请详情");
								var customerAid = _this.addAHtml(jsonData,"name","单击查看客户信息详情");
								...
								var customerParams = {customerId:CustomerId,applyId:_this.applyId,customerInfoId:baseId,parent : _this.appPanel}
								_this.viewDetailInfo(customerAid,custTabId,customerParams,url,custTitle);
								...
								var applyParams  = {id:applyId,applyId:applyId,customerId:CustomerId,custType:custType};
								_this.viewDetailInfo(applyAid,applyTabId,applyParams,applyurl,title);
//								jsonData["custType"] = "个人客户";
								...
								jsonData["name"] =  jsonData["name"]+","+jsonData["sex"]+","+jsonData["maristal"]/*+","+jsonData["custType"]*/;
								jsonData["cardType"] =Render_dataSource.gvlistRender('100002',jsonData["cardType"])+":"+jsonData["cardNum"];
								var appAmount = jsonData["appAmount"];
								jsonData["appAmount"] =  Cmw.getThousandths(appAmount)+'元&nbsp;&nbsp;<span style="color:red;"></br>(大写：'+Cmw.cmycurd(appAmount)+')</span>';
								jsonData["loanLimit"] = _this.getLoanLimit(jsonData)+","+ Render_dataSource.rateTypeRender(jsonData["rateType"])+","+jsonData["rate"]+'%';
								
//								_this.renderDispData(jsonData);
							}
						}
					}];
			var detailPanel = new Ext.ux.panel.DetailPanel({id:this.oneCustDetailPnlId,detailCfgs : detailCfgs_1,isLoad : false,border:false});
			return detailPanel;
		},
		addAHtml : function(jsonData,prop,title){
			if(!jsonData[prop]) return null;
			var aId = Ext.id(null,"VIEW_DETAIL_");
			var val = jsonData[prop];
			jsonData[prop] = "<a id='"+aId+"' href='javascript:void(0)'  class='hint--bottom' data-hint='"+title+"'>"+val+"</a>";
			return aId;
		},
		...
		getEntCustDetailPanel : function() {
			var _this = this;
			var htmlArrs_1 = [
					'<tr><th col="code">申请单编号</th> <td col="code" >&nbsp;</td><th col="name">企业名称</th> <td col="name" >&nbsp;</td><th col="kind">企业性质</th> <td col="kind" >&nbsp;</td></tr>',
					'<tr><th col="regaddress">注册地址</th> <td col="regaddress">&nbsp;</td><th col="breedName">业务品种</th> <td col="breedName" >&nbsp;</td><th col="loanLimit">贷款信息</th> <td col="loanLimit" >&nbsp;</td></tr>',
					'<tr><th col="appAmount">贷款金额</th> <td col="appAmount" >&nbsp;</td><th col="managerName">业务经理</th> <td col="managerName" >&nbsp;</td><th col="appdate">申请日期</th> <td col="appdate" >&nbsp;</td></tr>'];
			var detailCfgs_1 = [{
						cmns : 'THREE',
						/* ONE , TWO , THREE */
						model : 'single',
						...
						callback : {
							sfn : function(jsonData) {
								...
								var applyId = jsonData["id"];
								var applyAid = _this.addAHtml(jsonData,"code","单击查看贷款申请详情");
								var customerAid = _this.addAHtml(jsonData,"name","单击查看企业客户信息详情");
								var custTabId = CUSTTAB_ID.ecustomerInfoDetailTab.id;
								var url =  CUSTTAB_ID.ecustomerInfoDetailTab.url;
								var custTitle =  '企业客户详情';	
								var CustomerId = jsonData["customerId"];
								var baseId = jsonData["baseId"];
								var customerParams = {ecustomerId:CustomerId,applyId:_this.applyId,ecustomerInfoId:baseId,parent : _this.appPanel}
								_this.viewDetailInfo(customerAid,custTabId,customerParams,url,custTitle);
								...
								if(regcapital){
									...
									jsonData["regcapital"] =  Cmw.getThousandths(regcapital)+'元&nbsp;&nbsp;<span style="color:red;"></br>(大写：'+Cmw.cmycurd(regcapital)+')</span>';
								}else{
									jsonData["regcapital"] =  regcapital;
								}
								var kind = jsonData["kind"];
								jsonData["kind"] = Render_dataSource.gvlistRender('100009', kind)+",注册资金："+jsonData["regcapital"];
								var appAmount = jsonData["appAmount"];
								jsonData["appAmount"] =  Cmw.getThousandths(appAmount)+'元&nbsp;&nbsp;<span style="color:red;"></br>(大写：'+Cmw.cmycurd(appAmount)+')</span>';
								jsonData["loanLimit"] = _this.getLoanLimit(jsonData)+","+ Render_dataSource.rateTypeRender(jsonData["rateType"])+","+jsonData["rate"]+'%';
//								_this.renderDispData(jsonData);
			...
			var detailPanel = new Ext.ux.panel.DetailPanel({id:this.entCustDetailPnlId,detailCfgs : detailCfgs_1});
			return detailPanel;
		},
		renderDispData : function(jsonData){
			jsonData["custType"] = (jsonData["custType"]== "0") ? "个人客户" : "企业客户";
