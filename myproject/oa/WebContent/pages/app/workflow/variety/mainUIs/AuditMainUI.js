...
define(function(require, exports) {
	exports.viewUI = {
		...
		createCmpts : function(){
			...
			this.auditPanel = new Ext.Panel({title :'审批信息',height:500,autoScroll:true,border:false});
			this.bussMakePanel = new Ext.Panel({title :'业务办理',hidden:true,height:500,border:false
				});
			this.processImgPanel = new Ext.Panel({title :'流程预览',height:500,autoScroll:true,border:false});
		...
		resize : function(adjWidth,adjHeight){
			if(!adjWidth && !adjHeight){
				var adjWidth = this.tab.getWidth();
