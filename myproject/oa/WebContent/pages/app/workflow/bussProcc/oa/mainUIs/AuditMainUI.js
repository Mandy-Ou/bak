...
define(function(require, exports) {
	exports.viewUI = {
		...
		createCmpts : function(){
			this.topPanel = new Ext.Panel({border:false});
...
//			this.auditPanel = new Ext.Panel({title :'审批信息<span id="'+menuLabelId+'" class="buss_menu_list_menuLabel">XXXXXX</span>',height:500,autoScroll:true,menuLabelId:menuLabelId});
			this.auditPanel = new Ext.Panel({title :'审批信息',height:500,autoScroll:true});
			this.bussMakePanel = new Ext.Panel({title :'业务办理',height:500,hidden:true,autoScroll:true});
			this.processImgPanel = new Ext.Panel({title :'流程预览',height:500,autoScroll:true});
			this.tabPanel = new Ext.TabPanel({activeItem:0,autoScroll:true,border:false,items:[this.auditPanel,this.bussMakePanel,this.processImgPanel]});
			this.tabPanel.setActiveTab(0);
			var _this = this;
			this.tabPanel.addListener('render',function(tabPnl){
				tabPnl.hideTabStripItem(_this.bussMakePanel);
			});
			this.appMainPanel = new Ext.Panel({border:false,autoScroll:true,items:[this.topPanel,this.tabPanel]});
		},
		/**
		 * 组装控件
		 */
		...
		loadAuditMgrModInfo : function(){
			if(!this.auditMgrMod){
				...
				Cmw.importPackage('/pages/app/workflow/bussProcc/oa/mainUIs/AuditListMod',function(module) {
		...
		loadBussMgrModInfo : function(){
			...
			if(!this.bussMgrMod){
				...
				Cmw.importPackage('pages/app/workflow/bussProcc/oa/mainUIs/BussMenuListMod',function(module) {
		...
		loadProcessImgModInfo : function(){
			if(!this.processImgMod){
				...
				Cmw.importPackage('/pages/app/workflow/bussProcc/oa/mainUIs/ProcessImgMod',function(module) {
		...
		copyParams : function(data){
			...
			this.params.oaprocInstanceId = data.oaprocInstanceId;/*OA系统用到 2014-02-05 ADD*/
		},
		...
		resize : function(adjWidth,adjHeight){
			...
			if(adjHeight < 500) adjHeight = 500;
