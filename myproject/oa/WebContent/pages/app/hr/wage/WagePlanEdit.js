	...
	exports.WinEdit = {
		...
		getUrls : function(){
			...
			if(this.optionType == OPTION_TYPE.ADD){ //--> 新增
				...
				urls[URLCFG_KEYS.ADDURLCFG] = {url : './hrWagePlan_add.action',cfg : cfg};
			}else{
				/*--- 修改URL --*/
				...
				cfg = {params : {id:selId}};
				urls[URLCFG_KEYS.GETURLCFG] = {url : './hrWagePlan_get.action',cfg : cfg};
			}
			var id = this.appFrm.getValueByName("id");
			cfg = {params : {id:id}};
			/*--- 上一条 URL --*/
			urls[URLCFG_KEYS.PREURLCFG] = {url : './hrWagePlan_prev.action',cfg :cfg};
			/*--- 下一条 URL --*/
			urls[URLCFG_KEYS.NEXTURLCFG] = {url : './hrWagePlan_next.action',cfg :cfg};
			return urls;
		},
		/**
		 * 当数据保存成功后，执行刷新方法
		...
		createForm : function(){
			...
			var layout_fields = [
			txt_code, txt_name, txa_remark,hid_id,hid_Isenabled];
			var frm_cfg = {
			    title: '工资方案编辑',
			    url: './hrWagePlan_save.action'
			};
