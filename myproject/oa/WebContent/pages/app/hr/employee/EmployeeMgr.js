 ...
 */ 
cmw.skythink.EmployeeMgr = function(){
	this.init(arguments[0]);
}

Ext.extend(cmw.skythink.EmployeeMgr,Ext.util.MyObservable,{
	...
	getQueryFrm : function(){
		var self =this;
		var txt_areaName = FormUtil.getTxtField({fieldLabel : '员工名称',name:'name'});
 		var txt_name = FormUtil.getTxtField({fieldLabel : '所属部门',name:'indept'});
		 var layout_fields = [{cmns:'THREE',fields:[txt_areaName,txt_name]}]
	 ...
	 */
		...
		var barItems = [{
			...
			handler : function(){
				EventManager.enabledData('./hrEmployee_delete.action',{type:'grid',cmpt:self.appgrid,optionType:OPTION_TYPE.ENABLED,self:self});
			}
		},{
			token : "禁用",
			...
			handler : function(){
				EventManager.disabledData('./hrEmployee_delete.action',{type:'grid',cmpt:self.appgrid,optionType:OPTION_TYPE.DISABLED,self:self});
			}
		},{
			token : "删除",
			...
			handler : function(){
				EventManager.deleteData('./hrEmployee_delete.action',{type:'grid',cmpt:self.appgrid,optionType:OPTION_TYPE.DEL,self:self});
			}
		}];
	 ...
	 */
		getAppGrid : function(){
			...
			{
			    header: '员工状态',
			    renderer:Render_dataSource.emp_statusRender,
			    name: 'status'
			}];
			var _this=this;
				...
				if(_this.appCmpts[winkey]){
					...
					var winModule=null;
						if(winkey == Btn_Cfgs.MODIFY_BTN_TXT){
							...
							winModule="EmloyeeEdit";
						}else{
							winModule="EmloyeeAdd";
						}
					Cmw.importPackage('pages/app/hr/employee/'+winModule,function(module) {
